...
and CSS cases, including issue prose, asset URLs, SVG fragments, compound named colors, encoded
data-SVG colors, conditional values, and suppression behavior. It exercises every canonical CSS
named color in both languages to keep the rules' name sets aligned. Fixtures use unique temporary
names so concurrent lint runs cannot collide. The test runs after the normal Biome check as part
of `pnpm lint`.
