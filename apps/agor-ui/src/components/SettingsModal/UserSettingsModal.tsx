...
export const UserSettingsModal: React.FC<UserSettingsModalProps> = ({
  ...
  // Inactive tool values live in agenticConfigDraftByTool; only the active provider needs a form.
  ...
  // A tool-owned ProviderSettings contribution counts as an authentication pane
  // even when the generic environment-field registry is empty.
  useEffect(() => {
  ...
  }, [activeKey, activeTool, agenticForm, audioForm, open, providerSubtab, user]);
  ...
  const saveAgenticConfigs = async (tools: AgenticToolName[]) => {
    ...
    const valuesForTool = (tool: AgenticToolName): AgenticConfigFormValues => {
      ...
      if (activeTool === tool) return agenticForm.getFieldsValue() as AgenticConfigFormValues;
      return {
  ...
  const handleAgenticConfigSave = async (tool: AgenticToolName) => {
    ...
    try {
      if (toolsToSave.includes(tool)) await agenticForm.validateFields();
  ...
  const resolveProvider = useCallback(
    (tool: AgenticToolName) => {
      ...
      const authMethod = authMethodForTool(
        tool as TenantAgenticToolName,
        agenticAuthMethods,
        fieldStatus
      );
    ...
    const groups: Array<{
      ...
      },
      {
        ...
        children: visibleAgenticToolTabs.map((tool) => ({
          ...
          // Integration-owned credentials (OpenCode) cannot be inferred from
          // generic environment-field presence. Its panel renders authoritative
          // native status, so do not show a misleading host-owned dot here.
          status: getAgenticToolUIIntegration(tool)?.ProviderSettings
            ? undefined
            : resolveProvider(tool).status,
        })),
      },
      {
        key: 'grp-environment',
  ...
  const settingsIndex = useMemo<SettingIndexEntry[]>(() => {
    ...
    // Authentication tab exists when the tool has credential fields or an
    // integration-owned ProviderSettings contribution, the
    ...
    for (const tool of visibleAgenticToolTabs) {
      ...
      if (ProviderSettings || allToolFields.length > 0) {
        entries.push({
          label: ProviderSettings ? 'Providers' : 'Authentication',
          kind: 'setting',
  ...
  const renderProviderPanel = (tool: AgenticToolName) => {
    ...
    const statusTag = ProviderSettings ? null : status === 'connected' ? (
    ...
    if (ProviderSettings) {
      ...
      if (managesOwnSettings && client) {
        // Remount on authenticated-subject changes so no native provider state,
        // OAuth attempt, or in-flight response leaks between users.
    ...
    const effectiveSourceCaption =
      !managedByWorkspace && effectiveSource !== 'Unavailable' ? (
        ...
        >
          Effective source: {effectiveSource}. {policyDescription}
        </Typography.Paragraph>
      ) : null;
  ...
  return (
    ...
    >
      {/* Keep inactive shared forms connected. The active provider owns the
          single agentic form; inactive provider values live in draft state. */}
