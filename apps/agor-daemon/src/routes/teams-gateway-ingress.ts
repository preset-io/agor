...
function sameServiceHost(left: string, right: string): boolean {
  try {
    const lhs = new URL(left);
    const rhs = new URL(right);
    const port = (url: URL) => url.port || '443';
    return (
      lhs.protocol === 'https:' &&
      rhs.protocol === 'https:' &&
      !lhs.username &&
      ...
      !rhs.username &&
      !rhs.password &&
      lhs.hostname.toLowerCase() === rhs.hostname.toLowerCase() &&
      port(lhs) === port(rhs)
    );
  } catch {
    return false;
...
export function validateTeamsVerifiedIdentity(
  ...
  if (claimString(claims, 'channelid') !== 'msteams') return 'invalid_channel_endorsement';
  ...
  const serviceUrl = claimString(activity, 'serviceUrl');
  const tokenServiceUrl = claimString(claims, 'serviceurl');
  if (!serviceUrl || !tokenServiceUrl || !sameServiceHost(serviceUrl, tokenServiceUrl)) {
...
export function registerTeamsGatewayIngressRoute(input: {
  ...
  const limiter = rateLimit({
    ...
    legacyHeaders: false,
    keyGenerator: (req) => `${req.params.gatewayChannelId}:${req.ip}`,
  });
      ...
      try {
        ...
        await runWithTenantDatabaseScope(input.db, discovery.tenant_id, async (tenantDb) => {
          await new GatewayInboundEventRepository(tenantDb).admitVerifiedHttp({
            ...
            deliveryMetadata: normalized.metadata,
            address: {
