...
const gatewayChannelCreateSchema = z
  .strictObject({
    ...
    enabled: z
      ...
      .optional()
      .describe(
        'Whether the channel is active. Teams defaults to false; other providers default to true.'
      ),
    config: configSchema,
  ...
  .superRefine((value, issue) => {
    ...
    if (value.enabled ?? value.channelType !== 'teams') {
    ...
    if (value.channelType === 'teams') {
      const validation = validateTeamsConfig(config, {
        requireAppPassword: value.enabled === true,
      });
...
function toServiceCreateData(
  ...
  return {
    ...
    ...(args.agorUserId
      ...
      : {}),
    enabled: args.enabled ?? args.channelType !== 'teams',
    config,
...
export function registerGatewayChannelTools(server: McpServer, ctx: McpContext): void {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      const status = {
        // Fleet-wide workload counts are not tenant-owned diagnostics.
        status: {
