...
export const SYLLABUS: LessonMeta[] = [
  ...
  },
  {
    ...
    agent: true,
    script: [
      "Reopen lesson 04's conversation from the branch card — sessions are durable",
      'The agent proposed a concrete fix and asked to implement it — say yes',
      'Watch it edit for real; review the inline diff blocks in the transcript',
  ...
  },
  {
    ...
    agent: true,
    script: [
      'Create "Sprinkles" via Create New… → Teammate (framework repo pre-registered)',
      'Its first session starts bootstrapping the teammate for real — identity, memory, goals',
      'Teammates preside over their own board — end there, Sprinkles at the helm',
      'The bootstrap keeps working in the background while you move on',
    ],
    endsWith: "Sprinkles presiding over Sprinkles's Board from the left panel",
    notes:
      'Creating a teammate spawns its own board + a real bootstrap session (metered live). Cross-board assignment is blocked by a permission-override precondition (findings #12).',
  },
  {
    id: '10-mcp-tools',
    number: '10',
  ...
  },
  {
    ...
    agent: false,
    script: [
      'Create a second user (Ada) via Settings → Admin → Users — there is no email invite flow',
      'Ada joins the same board in her own browser; her live cursor + name chip appear',
      "Ada drags a branch card — it moves on the admin's screen in real time",
    ],
    endsWith: 'A visibly multiplayer board',
    notes: "Second context is driven but not recorded — the footage is the admin's point of view.",
  },
  {
