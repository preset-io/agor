...
The response is the patched `Task` row with `status: 'running'`. The endpoint only accepts tasks in `created` state on an `idle` session, while `queued` tasks drain automatically in queue-position order, and busy sessions should be prompted via `POST /sessions/:id/prompt` (which creates and queues the task atomically).
