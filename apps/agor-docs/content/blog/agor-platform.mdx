...
const client = createClient('http://localhost:3030', true, {
  socketAuthentication: { accessToken: '<signed-access-token>' },
});
...
const client = createClient(process.env.AGOR_DAEMON_URL, true, {
  socketAuthentication: { accessToken: process.env.AGOR_ACCESS_TOKEN! },
});
...
Read about [how Agor is built](/guide/architecture) - FeathersJS, Drizzle ORM, LibSQL, the whole stack.
...
- [Architecture Documentation](/guide/architecture)
- [REST API Reference](/api-reference)
- CLI command help: `agor --help`
