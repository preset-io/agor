...
4. **Tools.** Select suggested tools, or skip. Selections do not connect anything or grant access. After setup, Catalog opens the first selected entry for your new teammate’s branch; you can review permissions, connect it, and choose other tools there. “Browse the full catalog after setup” opens the full catalog instead. Skipping or deselecting every tool opens nothing.
...
When the board opens, a connected teammate's first session picks up where the wizard left off — it already knows its name, goals, and goal-tailored tool suggestions. Each suggestion carries its actual setup route: curated services go to **Catalog**, custom endpoints such as Slack go to **Settings → MCP Servers**, and GitHub MCP uses the reviewed **GitHub Catalog entry**, with a fine-grained personal access token entered securely in its drawer. Connecting a repository to Agor does not authenticate GitHub MCP. Never paste a PAT into chat. If teammate creation could not finish, open Catalog from the header once a branch is available. Workspace MCP policy may require an admin to enable or install a service. From there the teammate can register repos, organize its board, structure a workflow, or just be a thinking partner. **Anything a user can do in Agor, the teammate can do too.** It has full access to the [Agor MCP Server](/guide/internal-mcp). If you skipped AI setup, connect a model later from **Settings → AI & Agents** and start a session from the board.
