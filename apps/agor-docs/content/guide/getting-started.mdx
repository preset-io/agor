...
When the board opens, a connected teammate's first session picks up where the wizard left off — it already knows its name, goals, and goal-tailored tool suggestions. Each suggestion carries its actual setup route: curated services go to **Catalog**, custom endpoints such as Slack go to **Settings → MCP Servers**, and GitHub work uses a repository connected to Agor rather than inventing an MCP install. Workspace MCP policy may require an admin to enable or install a service. From there the teammate can register repos, organize its board, structure a workflow, or just be a thinking partner. **Anything a user can do in Agor, the teammate can do too.** It has full access to the [Agor MCP Server](/guide/internal-mcp). If you skipped AI setup, connect a model later from **Settings → AI & Agents** and start a session from the board.
