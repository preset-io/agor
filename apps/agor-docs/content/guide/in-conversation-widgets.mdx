...
They share one lifecycle, one security contract, and one auto-resume path. Further types (confirmations, MCP-server picker, file picker) would use the same plumbing.
...
Ask in a Slack thread and the card comes to you: Agor posts a **Connect _Notion_** button into the same thread, and edits that one message in place as the request moves on — signing in, connected, expired, cancelled, or replaced by a newer request. There is never a second button beside a stale one.
...
The agent is also handed a link to the Agor session and told to relay it. That is the fallback, and it always works: open it, click **Connect**, come back. You will see it if the card could not be posted, if an administrator has turned the Slack card off, or if you asked from a thread Agor is not configured to write into.

### If you asked from Discord, GitHub, or Teams
...
If the gateway channel does not map platform users to Agor accounts, every message in it runs as one shared account, so a sign-in started there would mint a credential the whole channel could drive: the agent refuses and names the setting to turn on, and the Slack card says the same thing in the thread if alignment is switched off after the card was posted. A card retired that way stays retired — re-enabling the setting does not make a weeks-old button clickable; ask again and you get a fresh one.
...
If you already hold a usable connection, no button appears. Agor attaches the server and resumes the agent immediately. A second request for the same server retires the earlier card rather than stacking a second button beside it.
...
| Browser → provider (`oauth`)      | **No value ever reaches the browser** — the provider redirects to Agor's own callback, which exchanges the code and stores the grant |
