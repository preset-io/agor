...
### If you asked from Slack, Discord, or GitHub

The card renders in the Agor transcript, which is not where you are. The agent is handed a link to the session and told to relay it — open it, click **Connect**, and come back.

Two things fail closed here. If the gateway channel does not map platform users to Agor accounts, every message in it runs as one shared account, so a sign-in started there would mint a credential the whole channel could drive: the agent refuses and names the setting to turn on. And the connection is always looked up and stored under **the person who is actually prompting** — never the session's owner.
