...
stores resurrection data beneath the effective Unix user's home. It preserves
...
The cache directory is mode `0700`. In
`simple` or `insulated` mode, users intentionally share a Unix identity and
therefore its home and resurrection data; use `strict` or a delegated
per-user runtime when that sharing is inappropriate.
