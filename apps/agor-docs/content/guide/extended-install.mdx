...
Migrations `0074_knowledge_embedding_claims` and `0078_mcp_oauth_pending_flows` change distributed ownership/state protocols. Existing PostgreSQL installations must apply either pending migration as a **stop-the-world cutover**. Rolling old and new daemons against the same database is not supported.
