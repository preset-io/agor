...
During `agor init`, select the agentic tools this deployment supports. Init records that selection in
the deployment-owned configuration and installs the packages. The original npm installation never
downloads optional runtimes. You can skip everything and add tools later by editing the file:
...
Then run `agor install` to reconcile disk state.
...
The packaged daemon refuses to start when a configured integration is absent, corrupt, or from a
different Agor version. Unconfigured tools are unavailable to tenants and do not block startup.
...
```bash
npm install -g agor-live@latest
agor init --agentic-tools claude,codex
```
...
`agor install` is declarative and idempotent. It installs missing or invalid configured packages,
...
Upgraded configurations that do not yet contain `agentic_tools.installed` fail safely. Add the tools
the deployment supports—or an explicit `installed: []` when it supports none—before running
`agor install`; absence is never interpreted as permission to delete every existing package.
