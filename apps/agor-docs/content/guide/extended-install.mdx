...
integrations behind and the new version starts with none. Reinstall the same set with `--restore`:
...
`--restore` reads the tools you had on your previous version and installs them at the new one. On a
machine with nothing installed yet it reports that there is nothing to restore and exits
successfully, so it is safe in unattended upgrade scripts.
...
```bash
agor install --restore --prune
```

Verify the result at any time with `agor doctor`.
