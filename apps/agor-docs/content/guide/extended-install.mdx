...
The in-app **Sign in with Claude** OAuth flow is enabled by default on supported
execution topologies. Operators can disable it with
`agentic_tools.claude_subscription_oauth: false`; see [Operator configuration](./config-yaml).
When disabled, Agor continues to support Anthropic API keys and manually pasted
`claude setup-token` credentials without exposing the OAuth
