...
Each tool may have one workspace default preset. Users can choose that moving workspace default, a specific preset, or (when policy permits) an inline personal default. Creation forms also offer **Use my default**. These default references are resolved when a session, schedule, or gateway channel is saved; the resource then stores the concrete preset reference or inline configuration. Later changes to a user's default do not silently rewrite existing resources.
