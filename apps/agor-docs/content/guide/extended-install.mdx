...
Microsoft Teams gateway support is bundled with the daemon and does not require a separate
`botbuilder` installation. Configure the Azure Bot credentials in the Teams gateway channel;
the daemon uses the Microsoft Agents SDK for callback authentication and delivery.
