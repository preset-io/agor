...
The additive `0109_user_provider_oauth_grants` schema introduces an incompatible
...
access token; use **Continue** after an expiry failure to start a new task while
preserving SDK resume state. No automatic prompt replay or billing fallback occurs.
