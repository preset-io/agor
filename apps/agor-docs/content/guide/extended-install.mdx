...
In trusted local installations using the built-in local `unix_user_mode: simple` executor (without an `executor_command_template`), Agor gives each user a stable tenant/user-scoped Codex home for ChatGPT device sign-in, login-file import, auth checks, logout, and Codex task execution. A task uses the session owner's Codex home, including when a trusted collaborator prompts a shared session, and Agor's path overrides any user-managed `CODEX_HOME` for that task. This prevents accidental credential and runtime-state sharing, but it is not a filesystem security boundary: every simple-mode executor still runs as the daemon account and can access every namespace. Sandbox, delegated, and templated-simple home routing are unchanged. Use `sandbox` rather than `simple` for multiplayer server isolation.
