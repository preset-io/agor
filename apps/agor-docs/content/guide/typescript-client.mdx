...
Requires Node ≥ 22.13 (or any modern browser). ESM and CJS builds are both included; TypeScript types are first-class.
...
- `getApiKeyFromEnv()` pulls `AGOR_API_KEY` from the environment
- `shortId(id)` renders the canonical short form Agor uses everywhere (URLs, pills, logs, notifications)
