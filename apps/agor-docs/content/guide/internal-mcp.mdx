...
| `agor_links_move` | Move a web or knowledge link to a session or branch owner |
| `agor_links_save` | Save a link to the current branch or through the guarded teammate-promotion flow |
| `agor_links_delete` | Remove the saved link without changing its target or source message |

Parsed and uploaded links keep their source-owned targets read-only. Their label and pin state remain editable after the link is persisted. Web and knowledge links can move between owners. File and internal links stay with their owner until their lifetime and access rules are defined.
