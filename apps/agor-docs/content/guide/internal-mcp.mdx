...
1. **While the child is still active**, call `agor_sessions_prompt` with
...
4. **ONLY THEN** call `agor_sessions_stop({ sessionId: childId, reason: "New data
...
```js
// First, while the child is active:
...
agor_sessions_stop({ sessionId: childId, reason: 'Obsolete approach; update U prioritized' });
```
...
before proceeding; do not blindly stop a newly dispatched update. Queue tools
