...
If the returned branch remains `creating` after its persisted filesystem-attempt deadline, a branch
manager with write filesystem access can call `agor_branches_recover_filesystem`. The tool fences the
exact expired attempt and reconciles the existing checkout; it does not launch a duplicate clone or
worktree mutation. Follow it with `agor_branches_wait_for_ready`. Active attempts and stale recovery
