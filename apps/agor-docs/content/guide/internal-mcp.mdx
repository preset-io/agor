...
External agents can use the same HTTP endpoint with a personal API key. See
[Connect an external MCP client](/guide/external-mcp) for verified Claude Code
and Codex configuration, secret handling, and troubleshooting.
...
- [Connect an External MCP Client](/guide/external-mcp) (Claude Code and Codex setup)
