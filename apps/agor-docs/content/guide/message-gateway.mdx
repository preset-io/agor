...
> Currently supports **Slack**, **Discord beta**, **GitHub**, **Shortcut**, and **Microsoft Teams**. WhatsApp and Telegram connectors are planned.
...
GitHub and Shortcut polling watermarks, and Discord Gateway sequence/session
checkpoints, are persisted behind the same owner fence. Discord resumes from
the ordered checkpoint when ownership changes. Their create-comment APIs do not
...
## Discord beta behavior

Discord beta is maintainer-only and intentionally narrow:

- Each enabled gateway row uses one Discord bot application, one guild, one or more configured public text channels, and one fixed `agor_user_id`.
- Inbound prompts are accepted from an allowed channel and existing public threads under that channel only. DMs, private threads, attachments, embeds, components, stickers, polls, webhooks, bot/self messages, wrong guild/channel messages, and unmentioned messages are ignored.
- Every prompt must explicitly mention the configured bot. A configured user or role allowlist must admit the author.
- The bot token is required and stored as a gateway secret. The beta requests only `GUILDS | GUILD_MESSAGES`; an explicit bot mention supplies Discord's content exception, so no privileged message-content setting is required. Legacy invalid Discord rows remain inert until corrected.
- Responses are sent as the bot, with generated mentions suppressed. Text is split into deterministic chunks of at most 2,000 Unicode code points, with Markdown fences reopened safely. Reply aliases for every chunk resolve to the same Agor session.
- Proactive sends are text-only, target `channel:<snowflake>`, and never create an automatic thread. A human reply to any outbound chunk consumes the existing durable outbound seed.

### Setting up Discord beta

Use the maintainer-only **Discord setup** wizard in Settings → Gateway
Channels, or the `agor_gateway_discord_setup` MCP tool for a secret-free
checklist. The setup flow does not create a Discord application or call
Discord. Create one application and bot in the Discord Developer Portal, then
invite it to the configured guild with permission to view channels, read
history, send messages, and reply in threads in each allowed public text
channel. The beta uses only `GUILDS | GUILD_MESSAGES`; explicit bot mentions
use Discord's content exception, so do not enable a privileged message-content
setting.

Enter the application, guild, and one or more allowed channel snowflakes plus at least one
allowlisted user or role. Select the fixed Agor user for the channel, enter
the bot token through the secure credential UI, and keep the row disabled until
the configuration is complete. If proactive outbound is enabled, use only
`channel:<allowed-channel-snowflake>` as the default target. Duplicate enabled
application IDs in one tenant are rejected fail-closed.

No live Discord QA is implied by the setup wizard or connection test. The
non-mutating probe can inspect identity, guild/channel access, shard
recommendation, and relevant permission bits, but cannot prove gateway event
delivery, role matchability for future authors, or end-to-end session creation
and reply. The beta does not support DMs, rich messages, attachments,
auto-created threads, webhooks, user alignment, or sharding.
...
- **Mentions are ignored.** The bot mention is required on every prompt. The author must also match an allowed user ID or hold an allowed role ID, and the message must be in the configured channel or an existing public thread beneath it.
- **Content is ignored.** DMs, private threads, attachments, embeds, components, stickers, polls, webhooks, and bot/self messages are outside the beta by design.
- **Gateway resume.** On PostgreSQL, Discord listener ownership, provider event IDs, and ordered Gateway checkpoints are durable and fenced by the channel tenant and listener lease. A stale or invalid configuration stays inert until corrected.
- **Proactive replies.** Use `channel:<snowflake>` for the configured channel. Replies to any response chunk use its durable alias and continue the same session; proactive sends do not create automatic threads.
