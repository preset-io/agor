...
Settings shows each channel's configured enabled state, creator, and execution
owner (or aligned sender). These are separate roles: the creator is not necessarily
the identity used to execute messages. Enabled does not certify a live connection.
Use **Edit** to change enabled state, inspect the target branch and last-message
time, and use the platform-specific connection checks. Gateway changes require an
administrator; deletion still requires confirmation.
