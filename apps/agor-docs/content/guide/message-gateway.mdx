...
> Currently supports **Slack**, **Discord**, **GitHub**, **Shortcut**, and **Microsoft Teams (Experimental)**.
...
Teams (Experimental) is disabled by default and uses a shared queue-first HTTPS ingress and a durable
...
4. Save the disabled draft. Supply credentials through the secure credential form, review the execution identity and allowlists, then explicitly enable the channel when ready for testing. The shared callback route accepts activities only while the channel is enabled; the durable worker processes admitted activities asynchronously.

Empty allowlists mean unrestricted access within the configured Microsoft tenant,
not deny-all. With no user map, accepted senders run as the configured fixed Agor
identity. Use a nonempty user map or narrow allowlists before enabling access.

### Upgrading an existing Teams channel

The migration disables every existing Teams channel. Re-enabling through the
channel settings advances its configuration generation. The old per-channel listener is not retained. Review the canonical
Microsoft Tenant ID, App ID, credentials, execution identity and allowlists, update
the Azure callback to the shared URL, and explicitly re-enable the channel.
Previously queued work and addresses with an older generation are not reused.
Databases used to test the earlier draft migration (before this migration was
renumbered) must be rebuilt from disposable data or repaired separately; do not
apply the new migration blindly to that unpublished draft schema.

Configuration revocation is fenced again at Task admission and atomically at the
durable outbound effect-start marker. A revocation that commits before that marker
prevents sending; revocation after an effect starts cannot retract the provider
request. An uncertain result stays ambiguous and is not automatically retried.
Keep Teams disabled during rollout and rollback; live provider QA remains required
before enabling it for real work. Apart from password-only rotation, configuration
changes conservatively invalidate queued work and stored addresses, including
catch-up tuning. Apply those changes while idle; the next verified activity
refreshes the address. This release does not promise queue continuity across policy
edits.

Idle workers back off their bounded database discovery to at most 60 seconds.
An activity can therefore be durably acknowledged before a worker starts it;
HTTP acceptance is not a promise of immediate Task execution.
