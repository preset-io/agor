...
Discord outbound emits use a `channel:<snowflake>` target, or `user:<snowflake>`
for a direct message when **Direct messages** is enabled. A server-channel outbound
...
- The channel and author boundaries are separate. Public parent channels,
  ...
  one author allowlist entry is required. Private threads, wrong guild or
  channel, bot/self messages, webhooks, embeds, components, stickers, polls,
  and server messages without a structured mention are ignored. DMs are off by default. Attachment handling
...
Enable **Direct messages** in the Discord settings (`direct_messages_enabled:true`,
or `directMessages:true` in the setup tool) to receive 1:1 DMs. This adds the
non-privileged `DIRECT_MESSAGES` gateway intent; it needs no extra portal toggle.
Group DMs and events without an explicit 1:1 DM type are ignored.

The sender must currently belong to the configured server and match an allowed
user or role. Membership and roles are looked up live for each message. A failed,
rate-limited, or timed-out lookup silently rejects that message without stopping
the listener. No mention is required. Identity alignment and the `files` setting
work as in server channels.

Each person's DM conversation uses one session on the target branch, without a
public thread. **DM content is visible to people with access to that branch.**
The first message posts the session link; follow-ups do not post routing notices.
There is no conversation-reset command.

For a proactive DM, call `agor_gateway_emit_message` with `target: "user:<snowflake>"`.
The channel must allow outbound messages, the caller must have emit permission,
and a calling session must belong to the target branch. The recipient must be a
current member of the configured server; unlike inbound DMs, they need not match
an author allowlist. Membership is checked live before opening the DM.

Failures are content-free and distinct: `discord_direct_messages_disabled`,
`discord_dm_target_not_member`, or `discord_dm_unreachable` (Discord refuses the
DM, including closed DMs or no mutual server). Refused sends are not recorded as
delivered or retried. Discord may rate-limit or block a bot that opens many DMs;
that can affect the whole bot. There is no bulk-send API.

The person's next eligible message, plain or a Discord reply, goes to their one
DM session—not the session that sent the proactive message. Earlier proactive
DM texts since the last admitted message are included from Agor's stored records
as untrusted context. The oldest texts are dropped first to fit the configured
prompt-byte limit, with the exact omitted count; the person's message is always
admitted. Several proactive messages do not create competing reply sessions.

Stored proactive context is included at most once after admission. A message
sent but not yet recorded when the reply is admitted can be missing from that
context; it remains in Discord and the sending session. Deleting the DM session
can cause older stored texts to appear again in the next new session.

DMs never use Discord history as prompt context. Messages sent while the listener
was down, DMs were disabled, or the sender was not allowed must be resent. Reply
delivery may read the latest 100 messages to recover a send by nonce; those texts
are discarded, not stored or used as context.

As in channel sessions, assistant replies and prompts marked as coming from Agor
are delivered to the DM, including UI/agent prompts and replies to callbacks or
widget continuations. A child-session callback prompt itself is not echoed.
Prompt labels name the task creator, not verified authorship. Anyone able to
prompt that session can make it write to the person even with proactive outbound
messaging disabled or after they leave the server. The gateway route method
cannot directly target DM mappings. Every send verifies the recorded channel is
still a 1:1 DM with that person; replies do not re-check server membership.

Turning DMs off takes effect after listener refresh. Queued tasks still run in
Agor, but their replies are not delivered to Discord. Existing sessions remain.
The connection probe reports the switch, but cannot verify end-to-end DM delivery.

### Catch-up and durability

For server channels, the live mention is the only wake-up trigger. After the mapping's last admitted
