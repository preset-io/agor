...
> **Teams ingress:** Microsoft Teams uses the shared
...
Teams uses a shared queue-first HTTPS ingress and a durable worker. It is safe
for load-balanced daemon replicas: the stable Teams activity ID is deduplicated
in `gateway_inbound_events`, claims can be recovered after expiry, and the
encrypted `ConversationReference` is refreshed by verified inbound activity.
...
- **Conversation mapping.** Personal and `groupChat` messages map to the whole conversation. Standard-channel messages map to the root reply chain. Channel messages require `@mention` by default; the mention is removed before the prompt.
...
best-effort, never blocks the current mention, and is not advertised as a
Graph-history correlation feature until a later provider QA can prove that
contract. Use `agor_gateway_teams_setup` for the manifest and
...
- **Catch-up did not run.** Standard-channel catch-up requires the recommended RSC permission and is bounded best-effort. It never blocks the current mention and may be unavailable; Agor does not claim Graph correlation from it.
