...
Bulk archive never follows cross-branch creation links. To archive a gateway parent and all its nested descendants, use the dedicated session Archive action: it includes remote-created Sessions by default. Archiving the parent branch also includes those descendants. Each affected branch requires permission; a denial leaves the archive unchanged. Restoring the parent remains branch-local, so restore remote Sessions separately.
