...
3. Optionally configure the team, channel, and user AAD-object allowlists, the Teams AAD-object-to-Agor-email `user_map`, mention behavior, bounded catch-up limits, and outbound delivery.
...
- **Bot doesn't respond to channel mentions.** Channels require `@mention` by default. Confirm the app has `team`/`groupChat` scope and that the team/channel/user allowlists include the activity.
