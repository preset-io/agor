...
HTML-first vanilla artifacts that have an empty (or comment-only) JavaScript entry are served through Sandpack's `static` template automatically. This keeps the original `index.html` markup, inline styles, linked CSS, and scripts intact while preserving the normal vanilla path for artifacts with executable JavaScript.
