...
- `agor_runtime`: Enables agent DOM introspection via `agor_artifacts_query_dom` by default. Opt out per artifact with `{ enabled: false }`, especially when rendering sensitive output.
