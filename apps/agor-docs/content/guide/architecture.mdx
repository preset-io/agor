...
| Tool | Description | Example |
|------|-------------|---------|
| `agor_tasks_list` | List tasks or pending queue in dispatch order | `{ sessionId: '...', status: 'queued' }` |
| `agor_tasks_cancel_queued` | Atomically remove selected queued tasks | `{ sessionId: '...', taskIds: [...] }` |
| `agor_tasks_reorder_queued` | Reorder the exact pending queue with stale-order protection | `{ sessionId: '...', expectedTaskIds: [...], taskIds: [...] }` |
