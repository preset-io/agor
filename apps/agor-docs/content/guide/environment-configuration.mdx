...
logs. When GitHub reports the Codespace fully shut down, Logs skips SSH-backed
calls so reading logs does not wake billable compute. Nuke revalidates and
deletes only the bound Codespace; use it when the disposable disk and credentials
must be recreated.
