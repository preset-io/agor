...
Agor's Logs action returns a bounded snapshot of that creation log even while
`postStartCommand` is running. Once the Codespace is available, it also retrieves
the nested Compose log tail over authenticated SSH. From a Codespace terminal,
you can inspect the stack directly:
