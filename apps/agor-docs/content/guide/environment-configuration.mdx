...
daemon can expire a claim whose owner vanished, advance its generation, and
make retry safe. Process groups are terminated with TERM then KILL, and Agor
does not settle until the process group is absent (or explicitly reports
containment as unverified). Generation-fenced settlement prevents a late executor from
overwriting a newer lifecycle action.
