...
changed, so this does not require a nightly job for normal stop/start cycles.
...
so a prebuild cannot absorb this nested image build. A future faster cold path
should publish a dedicated development cache/image only from trusted main and
consume it by immutable identity; the current production image is not that
development cache.
