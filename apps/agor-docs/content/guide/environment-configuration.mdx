...
any command at all. The same credential rule applies to Start: its effective
persisted startup deadline is shortened before the lifecycle generation is
opened, so a configured 24-hour startup can never outlive a 24-hour command
credential plus settlement time.
...
Automatic coordination is active only while a non-archived environment is
`starting`, `running`, or `stopping`. Readiness observations are recorded during
Start but cannot promote the durable state until the generation-fenced Start
callback has cleared its active provider-mutation attempt. `stopping` has no
health probe; it remains scheduled so a daemon restart can recover an expired
Stop/Nuke deadline. Terminal status, archive, and deletion remove the
environment from monitoring.
...
stopped, archived, or deleted environment. It may return a transient
...
In standalone mode those commands use the configured local executor boundary.
In HA, `hybrid` is accepted only for the external delegated executor topology;
it is rejected for `shared-local` so repository commands never fall back into a
daemon replica. Selecting a repository-defined variant follows normal branch
`all`/admin permissions. Injecting raw replacement shell text is a distinct,
admin-only operation; the variant picker must not become a shell-authoring
escape hatch.

Provider-backed variants should set explicit budgets and make every lifecycle
command retry-safe:
...
environment:
  ...
  variants:
    remote:
      startup_timeout_ms: 1500000
      lifecycle_timeout_ms: 1260000
      start: ./scripts/start-github-codespaces.sh
      stop: ./scripts/stop-github-codespaces.sh
```

`startup_timeout_ms` may be 1 second through 24 hours; the default is one hour.
`lifecycle_timeout_ms` may be 1 second through one hour. Its default is derived
from the bounded taskless executor credential. Do not use Start to keep an
application server in the foreground: Agor terminates the entire command
...
In daemon HA mode, `webhook-only` works with either supported execution
topology. `hybrid` is admitted only with `execution_topology: external`,
`unix_user_mode: delegated`, a command template, persistent branch/user storage
assertions, and the authenticated external executor-response protocol. Shell
lifecycle commands therefore go through the same delegated launcher and
concurrency admission as agent runs; they never execute inside daemon Pods.

Start has a persisted startup deadline. Stop, Nuke, Sync, and Restart's verified
Stop phase use the variant's bounded `lifecycle_timeout_ms`; the shell deadline,
credential lifetime, response waiter, and durable Sync lease derive from that
one budget. Stop/Nuke also persist a fleet-visible lifecycle deadline: another
daemon can expire any shell, webhook, or fallback claim whose owner vanished,
advance its generation, and make retry safe. Process groups are terminated with TERM then KILL, and Agor
does not settle until the process group is absent (or explicitly reports
containment as unverified). Generation-fenced settlement prevents a late
executor from overwriting a newer lifecycle action. Executor callback tokens
are bound to the exact action, branch, and lifecycle generation, so a still-live
token from an older attempt cannot replay into a newer one.

A completed non-success webhook response is authoritative. A timeout,
disconnect, or response-stream failure is not: the orchestration service may
still be applying the request. Agor records the diagnostic but retains the
lifecycle attempt or Sync lease until its durable deadline instead of admitting
an overlapping retry.

Provider mutations are serialized per branch. Stop/Nuke cannot preempt an
in-flight Start, and no lifecycle action can overtake a live Sync lease; wait
for the bounded attempt to settle or expire. This avoids a late Start recreating
a resource after Stop/Nuke. The commands themselves must still be idempotent
reconcilers: a retry after a Pod or daemon loss must converge the provider to
the requested state rather than blindly creating another resource.

Archiving or deleting a branch is also fenced against provider work. If its
environment is running, the first request initiates Stop and asks the user to
retry after Stop settles. A branch that is starting, stopping, or owned by an
active lifecycle/Sync attempt cannot be retired. This deliberately avoids
discarding the only recovery record for an external, potentially billable
resource.

Agor Cloud's supported pattern is a short-lived command such as
`start-github-codespaces.sh`: create or reconcile a remote environment, emit
the bounded `AGOR_ENVIRONMENT_RESULT`, and exit. A foreground application server
inside the transient executor Pod is killed at the Start deadline and is not
reachable—Cloud does not currently provide application ingress/proxying for
environment executor Pods. Remote-provider access controls, secrets, quotas,
and billing remain the operator's responsibility. Right-sized lifecycle
containers and application network wiring are planned separately.

### Why does the Environment tab show an instance-specific disclaimer? {/* #operator-notice */}

Deployment operators can configure constrained Markdown at the top of the tab
to explain where environments run, which integrations are supported, or where
to find local documentation. The disclaimer is informational UI policy; it does
not enable, disable, or authorize any environment operation.

It appears in addition to the dynamic **Managed environments** or
**Webhook-managed environments** summary. That summary comes from the daemon's
effective execution mode and remains the authoritative description of accepted
lifecycle fields. If no disclaimer is configured, the tab behaves exactly as it
did before this option was added.

See [Environment tab guidance](/guide/config-yaml#environment-tab-guidance) for
the `ui.environment_disclaimer_markdown` schema, limits, sanitization, public visibility,
