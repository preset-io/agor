...
### Why does the Environment tab show an instance-specific notice? {/* #operator-notice */}

Deployment operators can configure a plain-text notice at the top of the tab
to explain where environments run, which integrations are supported, or where
to find local documentation. The notice is informational UI policy; it does
...
lifecycle fields. If no notice is configured, the tab behaves exactly as it
did before this option was added.

See [Environment tab guidance](/guide/config-yaml#environment-tab-guidance) for
the `ui.environment_notice` schema, limits, safe-link rules, public visibility,
