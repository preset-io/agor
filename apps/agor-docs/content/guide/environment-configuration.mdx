...
environment:
  ...
  variants:
    <variant-name>:
      ...
      lifecycle_timeout_ms: 775000 # optional; budget for one stop/nuke/sync
...
`startup_timeout_ms`, `lifecycle_timeout_ms`, `sync`, `nuke`, `logs`, `health`,
and `app` are optional. The startup timeout defaults to one hour and may be set
from 1 second through 24 hours. Agor persists an exact deadline for every
attempt, so a daemon restart or a pause in health monitoring does not restart
the clock. The corresponding optional UI buttons simply hide or no-op when
absent.

`lifecycle_timeout_ms` is the separate budget for one `stop`, `nuke`, or `sync`
command; Start is governed by `startup_timeout_ms` instead. It ranges from 1
second to 1 hour and inherits through `extends`.

Leave it unset unless a variant genuinely needs longer. The default is not a
round number picked by hand: it is the largest command deadline whose executor
credential still fits inside the 15 minutes Agor gives a taskless executor
command by default, so an ordinary local `docker compose down` asks for no more
authority than it already had. Raising it raises three things together — the
credential that command is issued, how long the daemon waits for its result, and
the durable Sync claim lease. That last one is why the ceiling is an hour rather
than the 24 hours `startup_timeout_ms` allows: the lease is how long a peer
daemon must wait before it may retry a sync whose owner died.

Two operator settings bound this from outside, and Agor applies both when it
dispatches rather than letting the command discover them later:

- `execution.session_token_expiration_ms` (24 hours by default) caps any
  executor credential.
- `execution.executor_response.timeout_ms.by_command['environment.lifecycle']`,
  if set, caps how long the daemon waits for the command's result.

If either is tighter than the variant asks for, the **command deadline is
shortened** to fit — the command still runs, and still ends before its authority
and its waiter do. Agor only refuses outright when a ceiling leaves no room for
any command at all.
...
that revision to a clean checkout, reconciles the Compose stack, waits for
health, and re-verifies the remote HEAD before acknowledging success. The
development image is rebuilt only when its verified build inputs changed.
...
#### Operating the Codespaces example

Before pressing Start, push the branch ref to GitHub, register the repository
from a `github.com` remote, and authenticate the Agor host's current user with
`gh` and Codespaces access. The checked-in managed devcontainer installs the SSH
server required by the launcher's health, Logs, and Sync operations. A Codespace
created from an older devcontainer may need a devcontainer rebuild or a Nuke and
fresh Start before those operations can connect.

This example deliberately makes ports 3000 and 5000 public after the remote
health check succeeds. GitHub's forwarding tunnel adds no authentication to a
public port, and port 3000 exposes the complete Agor daemon rather than only its
health endpoint. The bootstrap therefore creates a unique admin password in the
Codespace. Retrieve it from a Codespace terminal, use it for the first login,
...
Start reconciles the branch's existing bound Codespace: it creates or resumes
one when necessary, revalidates its owner, repository, ref, and binding, repairs
an unhealthy nested Agor stack, and republishes current URLs after health is
ready. Pressing Start again after an interrupted attempt reuses that resource
rather than creating another one. GitHub state transitions can take several
minutes. Start keeps a bounded 24-minute provider wait inside Agor's 25-minute
lifecycle deadline, leaving a minute for the result to settle. Stop and Nuke
keep bounded 20-minute waits instead of reporting completion before GitHub
reaches the requested state. The variant therefore sets `lifecycle_timeout_ms`
to 21 minutes, which gives Stop, Nuke, and Sync a process deadline just past
that provider wait; the credential, the daemon's wait for the result, and the
Sync claim lease are all derived from it. Restart runs that bounded Stop first
and starts again only after the Stop has actually settled, so a slow provider
transition never needs one long-lived credential covering both phases.

Sync is the explicit source-update operation. It accepts a full commit ID that
has already been pushed to the configured ref, refuses to overwrite a dirty
Codespace checkout, applies exactly that commit, restarts the stack when needed,
waits for health, and verifies the applied commit before reporting success.
Stop shuts down the Codespace but preserves its disk and nested SQLite data.
Logs returns bounded creation diagnostics and, once available, nested Compose
logs. While GitHub reports the Codespace shutting down or fully stopped, Logs
skips SSH-backed calls so reading logs cannot delay shutdown or wake billable
compute. Nuke revalidates and deletes only the bound Codespace; use it when the
disposable disk and credentials must be recreated.

If GitHub cannot find the ref, push it before retrying. If the launcher reports
a missing SSH server, rebuild the managed devcontainer or Nuke and recreate the
Codespace. A public-port policy error means the repository or organization does
not allow the visibility requested by this example. If Sync reports a dirty
checkout, preserve or remove those in-Codespace changes yourself before retrying;
Agor will not overwrite them.
If the bootstrap password file is lost while persistent Codespace data remains,
restore that file or Nuke the disposable Codespace; Start refuses to invent a
replacement that would not match the existing admin account.
