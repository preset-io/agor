...
Agor's managed environments let a repo ship a **library of named variants** (`lean`, `postgres`, `rich`, …) that every branch can pick from. Variants are Handlebars-templated and get **rendered into a per-branch snapshot** that the Start/Stop/Nuke/Logs buttons execute against.
...
```text
  ...
  │    environment.variants.{lean,postgres,rich,…}         │
  └───────────────────────────┬────────────────────────────┘
...
A variant may extend **one** base variant. The base variant itself must **not** have an `extends` key. Chains (`rich → postgres → lean`) are rejected at save time with:

```text
variant 'rich' extends 'postgres', which itself extends 'lean' —
...
### Worked example for Superset: `lean` / `postgres` / `rich`
...
environment:
  ...
  variants:
    ...
    rich:
      ...
      start: 'COMPOSE_PROFILES=rich docker compose -p agor-{{branch.name}} up -d --build'
      stop: 'docker compose -p agor-{{branch.name}} down'
      nuke: 'docker compose -p agor-{{branch.name}} down -v'
      logs: 'docker compose -p agor-{{branch.name}} logs --tail=100'
```

Notice that `postgres` and `rich` both extend `lean` (not each other). That's the single-level rule. Both inherit `health` and `app` from `lean` and override the rest.
