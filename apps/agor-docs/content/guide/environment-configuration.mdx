...
one budget. Process groups are terminated on deadline, and generation-fenced
settlement prevents a late executor from overwriting a newer lifecycle action.
The commands themselves must be idempotent reconcilers: a retry after a Pod or
daemon loss must converge the provider to the requested state rather than
blindly creating another resource.
