...
- **Strict MCP OAuth by default**: Agor binds protected-resource and authorization-server metadata, sends the protected `resource`, requires PKCE S256 and RFC 9207 callback issuer identification, and requires HTTPS endpoints. Exact loopback HTTP is allowed only for standalone/SQLite development; PostgreSQL and hosted deployments reject it. Providers that lack RFC 9207 can use the explicit **Issuer-distinct callback compatibility** policy. That policy binds the flow to a stable issuer-specific callback path, but it is weaker than RFC 9207: RFC 9700 documents a Dynamic Client Registration attack that can bypass distinct redirect URIs. Use it only when strict mode is unavailable. A separate narrow **Legacy compatibility** setting also enables non-standard discovery; Agor never silently downgrades.
- **Client registration**: pre-registering an OAuth client is preferred. By default, Agor uses Dynamic Client Registration only when validated authorization-server metadata explicitly advertises an RFC 7591 registration endpoint. Guessing a legacy issuer-relative `/register` endpoint requires an explicit per-server fallback setting. An issuer-distinct compatibility flow uses `/mcp-oauth-v2/callback/issuer/<issuer-hash>` rather than the standard callback; its OAuth-start response returns the exact `redirectUri` to register for a pre-registered client.
