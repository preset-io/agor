...
execution:
  ...
  # Required to verify remote workload cleanup before any terminal release.
  executor_stop_command_template: |
    kubectl delete pod executor-{task_id} --ignore-not-found --wait=true --timeout=8s
```
...
| `{executor_attempt_id}` | Launch attempt ID (prompt executors) | `01def456...` |
| `{termination_reason}` | Stop reason (stop template only) | `user_stop` |
...
For remote/container execution, the start command is only the launcher. Configure
`executor_stop_command_template` so the daemon can delete the actual workload during
completion, explicit Stop, lease loss, launch failure, or restart recovery. The stop
command should be idempotent and return successfully only after it has verified that
the workload is gone. If the command fails or times out, Agor keeps the session fenced
and does not drain its queue.

For local executors running as the daemon user, restart recovery verifies the inherited
attempt marker through `/proc` on Linux. Other platforms and impersonated local workloads
remain fenced after an unclean daemon restart because Agor cannot prove that the previous
process tree is gone safely.

The daemon also fences launches that never connect. The default deadline is two
minutes and can be adjusted independently of heartbeat monitoring:
...
Setting `executor_heartbeat.enabled: false` disables running-task heartbeat expiry;
the dispatch connection deadline remains active so launch failures cannot stay in
