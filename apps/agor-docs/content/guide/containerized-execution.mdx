...
branch owner or administrator explicitly force-fails it. OpenCode server-side
execution also remains unverified. A stale heartbeat does not authorize the
daemon to signal a local executor that is still present; it rechecks until
process absence is verified. After a verified failure, Agor may continue the
next distinct Task already in the session queue, but it never retries or replays
the failed prompt. Unverified containment keeps queued work blocked. After an
abrupt daemon restart, Agor keeps its existing orphan cleanup behavior;
diagnostics disclose that logical release does not prove remote process
termination.
