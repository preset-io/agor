...
Shared storage can provide consistent workspace files across replicas in the
`shared-local` topology, but does not make the daemon stateless or remove HA
prerequisites. Active-active mode requires PostgreSQL, Redis, and sticky Engine.IO
ingress; SQLite on a shared volume is not a supported substitute. External
execution has a different storage contract. Follow the
[HA topology requirements](/guide/daemon-ha) for your deployment.
