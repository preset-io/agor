...
execution also remains unverified. After a verified failure, the user can prompt
again manually; the watchdog does not replay the prompt or automatically drain
queued work. After an abrupt daemon restart, Agor keeps its existing orphan
cleanup behavior; diagnostics disclose that logical release does not prove
