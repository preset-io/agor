...
pulse clears the active warning while preserving the recorded diagnosis.
...
For locally spawned executors, enforced failures enter `STOPPING` and wait for
bounded process-group termination and absence verification. Verified SDK health
containment settles as `FAILED`; user-requested Stop settles as `STOPPED`. If
process absence cannot be verified, the task remains `STOPPING` and the session
stays blocked rather than claiming successful containment. Templated/remote
execution that cannot prove absence likewise remains visible and blocked until a
branch owner or administrator explicitly force-fails it. This can add the
configured grace and signal-escalation latency. After a verified failure, the
user can prompt again manually; the watchdog does not replay the prompt or
automatically drain queued work. After an abrupt daemon restart, Agor keeps its
existing orphan cleanup behavior; diagnostics disclose that logical release
