...
The daemon includes the configured `daemonUrl` in executor payloads.
...
<a id="current-status" />
<a id="shared-filesystem-simplifies-ha" />
<a id="what-still-needs-redis" />

Containerized executors do not require multiple daemon replicas. For active-active
daemons, follow [Daemon High Availability](/guide/daemon-ha) for configuration,
support limits, and rollout instructions. PostgreSQL, Redis realtime fanout, and
sticky Engine.IO ingress are required; SQLite on shared storage is not supported.
Redis distributes notifications, not durable task locks or tenant authority.

In `shared-local`, workspace files must be consistent across replicas. External
execution instead requires persistent per-user homes and per-branch workspaces;
daemon pods do not need tenant workspace mounts. If the base repository is
unavailable, use clone-only branch storage. See the
[executor storage contract](/guide/daemon-ha#executor-storage-contract) before
choosing a topology.
