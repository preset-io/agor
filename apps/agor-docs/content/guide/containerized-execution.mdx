...
pause the applicable deadline, and meaningful SDK events reset the applicable
progress deadline. Tool and item starts do not create open-ended leases: if an
SDK never emits completion or further progress, supervision still resumes.
...
The Codex adapter normalizes reviewed SDK item lifecycles into progress.
Commands, MCP calls, web searches, file changes, reasoning, agent messages, and
todo-list changes reset the deadline without suspending it. New item kinds
