...
For local executors, restart recovery verifies the inherited attempt marker through
`/proc` on Linux. Other platforms remain fenced after an unclean daemon restart because
Agor cannot prove that the previous process tree is gone safely.
