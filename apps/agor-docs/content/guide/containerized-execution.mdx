...
| --------------------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------- |
| **Durable branch workspace**                              | Same branch bytes for every relevant invocation | Per-branch PVC/volume or a shared filesystem                  |
| **Persistent user home when native credentials are used** | Same isolated home for auth helpers and Tasks   | Per-user PVC/home selected from trusted tenant/user identity  |
| **Consistent execution identity**                         | UID/GID or delegated identity maps predictably  | Orchestrator identity mapping, LDAP/sssd, or synchronized IDs |
...
Agor's `insulated`/`strict` Unix isolation features require containers to resolve these IDs to
names. A hosted launcher can instead use `unix_user_mode: delegated`, where the substrate enforces
the identity and mounts rather than the daemon impersonating a host account.
...
making progress. The watchdog defaults to enforced supervision for recognized,
actionable SDK failures:

```yaml
execution:
  sdk_watchdog:
    mode: enforce # disabled | observe | enforce
    first_progress_timeout_ms: 180000
    operation_absolute_timeout_ms: 14400000 # 4h; never extended by progress
    abort_grace_ms: 15000
    claude_idle_timeout_ms: 3600000 # null disables Claude's post-progress check
    codex_idle_timeout_ms: 3600000 # null disables Codex's post-progress check
```
...
pause quiet deadlines, and meaningful SDK events reset only their applicable
quiet deadline. Identified tool, item, and background work is tracked per
operation with an absolute deadline; progress cannot extend that deadline.
...
It does not rearm the first-progress deadline. Agentic-tool post-progress
detection is enabled for Claude and Codex by default through their respective
idle timeouts. Both default to one hour; set either value to `null` to disable
that tool's general post-progress check. The same setting supplies the default
quiet deadline for identified operations. With `null`, operation quiet
supervision falls back to the four-hour absolute ceiling.

The Codex adapter normalizes reviewed SDK item lifecycles into identified operations.
Commands, MCP calls, web searches, file changes, reasoning, agent messages, and
todo-list changes have independent start/progress/finish facts. New item kinds
remain unknown activity until reviewed.
...
Cursor stream messages participate in first-progress supervision; its
post-progress deadline remains disabled because no Cursor-specific idle timeout
is configured.

SDK activity facts share the existing executor heartbeat transport. Disabling
`execution.executor_heartbeat.enabled` disables periodic liveness writes and the
stale-wrapper backstop. Actual SDK pulses remain coalesced at
`interval_ms` and persisted so observe-only recovery and diagnostics continue
to work. The executor-local watchdog and its direct failure report also remain
active.
...
abort its agentic-tool runtime and run cleanup regardless of which host or pod runs the
...
For locally spawned executors, enforced watchdog failures enter `STOPPING` and
wait for bounded process-group termination and absence verification. A short
cooperative grace is followed by SIGTERM/SIGKILL only when the wrapper does not
exit itself. Verified SDK health containment settles as `FAILED`; user-requested
Stop settles as `STOPPED`. If process absence cannot be verified, the task
remains `STOPPING` and the session stays blocked rather than claiming successful
containment.

A scoped remote executor's quiescence report is the authoritative cooperative
result because the daemon cannot inspect a process group on another host.
Missing reports still flow through the fleet runtime reconciler and remain visible
and blocked rather than being treated as proof of termination. Templated or
remote execution that cannot prove absence likewise remains blocked until a
branch owner or administrator explicitly force-fails it. Normal cooperative
completion uses the execution-mode proof above. If forced runtime cleanup cannot
verify that server-side work ended, Agor records `runtime_cleanup_unverified`;
local wrapper absence alone still cannot release the Task. Heartbeat expiry may
initiate scoped containment, but only verified local absence or fenced remote
quiescence permits release. After a verified failure, Agor may continue the next
distinct Task already in the session queue, but it never retries or replays the
failed prompt. Unverified containment keeps queued work blocked. At startup,
Agor claims `daemon_restart`, resumes containment, and keeps unverifiable work
`STOPPING`.
