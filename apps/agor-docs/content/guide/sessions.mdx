...
The conversation opens the latest five tasks with user and assistant text visible. Scroll
...
For newly finished turns, a verified recorded count hides the disclosure when zero and
...
reads/parses the stored message JSON. Five tasks is not a byte cap, and loading more
history retains it in memory. Reconnecting refreshes reached history and missed new
turns; it does not reset the reader to ten tasks.
