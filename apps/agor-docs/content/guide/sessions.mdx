...
## Lean transcript experiment (development only)

This branch includes an opt-in prototype, not a saved display preference. Run this
branch’s daemon and set `VITE_LEAN_TRANSCRIPT_POC=true` for its UI process to try it;
unset the flag to restore the existing transcript. No database migration is needed.
The flag is read when Vite starts; it does not start a managed preview environment.

The prototype opens the latest ten tasks with user and assistant text visible. Scroll
up (or use **Load older history**) to fetch earlier tasks. **Load tool activity** fetches
...
Turn metadata is available through the prompt’s metadata button, hover, or keyboard focus.
