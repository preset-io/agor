...
Agor may shorten tool inputs, results, or diff data in the displayed transcript
so the complete message write fits its transport budget. Shortened tool entries
show a notice and the original serialized field sizes. Inputs may be omitted
entirely, and shortened entries use a plain display rather than a full diff
preview. This changes only the persisted/displayed transcript, not the arguments
executed by the tool or the provider's conversation state. Prefer pagination,
filtered results, and files over large inline state snapshots.
