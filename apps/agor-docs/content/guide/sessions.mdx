...
- **Detailed** shows every task's metadata pills, bordered thinking blocks, and
  the full subagent summary with its tool tags.
- **Compact** keeps your prompts and the agent's written answers in view and
  folds the rest into quiet one-line rows: thinking, each tool call, and a whole
  subagent chain (`Worked for 1m 2s · 21 steps · 3 files`). Task metadata
  becomes a single muted `model · duration` line whose tooltip carries the rest.
