...
### Transcript view: Compact and Detailed

The session view renders a transcript in one of two views, chosen from the
session **⋯** menu under **Transcript view**:

- **Detailed** groups each prompt into its own collapsible task section, with
  metadata pills, bordered thinking blocks, and the full subagent summary.
- **Compact** reads as one continuous conversation with no per-prompt section to
  open. Your prompts and the agent's written answers are always fully visible;
  only the work in between folds into quiet one-line rows — thinking, each tool
  call, and a whole subagent chain (`Worked for 1m 2s · 21 steps · 3 files`).
  Task boundaries are a thin separator carrying `model · duration`, whose
  tooltip holds the tokens, context, git ref, and author.

Nothing is hidden permanently — every collapsed row expands in place.

The choice is stored on your user account, so it applies to every session you
open, on desktop and on a phone alike. Accounts created before Compact shipped
stay on Detailed until they pick otherwise; newer accounts start on Compact.
