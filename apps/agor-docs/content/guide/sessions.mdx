...
This reuses the existing threshold: more than 15 source lines, showing 10 when
collapsed (code fences can extend that preview). Wrapped single-paragraph prose
is not counted as multiple source lines. All fetched text remains available for
expansion and copying; this is a display preference, not a payload reduction.
