...
`AGOR_REDIS_KEY_PREFIX`, and the `AGOR_REDIS_*_MS` timeout/backoff variables. Storage guarantees
...
| --- | --- | --- | --- |
| Healthy | 200 | 200 | Both Redis clients are ready, the adapter is attached, and PostgreSQL answers. |
...
| Redis/adapter lost after boot | 200 | 503 | The process stays live for diagnosis but must leave ingress rotation. Already-committed PostgreSQL writes remain durable; notifications during the gap are not replayed. |
...
| --- | --- |
| Stateless REST and stateless MCP | Supported, subject to normal auth/tenant policy. Stateful MCP session IDs, GET, and DELETE fail closed. |
...
| Managed environments | Local commands are startup-blocked; use webhook-only. The legacy unfenced environment HealthMonitor is not constructed in HA and authenticated health reports `environmentHealthMonitor: false`. External webhook execution and its own monitoring plane must provide the production behavior. |
...
| Codex auth-file check/import/logout | Enabled only when `executor_storage.user_home` is `shared` or `persistent-per-user`; the operation runs through the executor and Redis never carries the file/tokens. The Compose profile uses one shared simple-mode Unix home; Cloud must use tenant/user-isolated persistent homes. |
...
2. Apply all migrations before admitting new-version daemons. Use an all-at-once cohort replacement; old executor tokens without authority rows fail closed.
...
To roll back, stop the HA cohort, remove all but one daemon from ingress, set `deployment.mode: standalone`, and restart that daemon against PostgreSQL. `REDIS_URL` may stay present because standalone ignores it. Do not rotate shared secrets or switch database engines during an emergency rollback. Process-affine features should be re-enabled only after the all-at-once transition is complete.

## Opt-in integration harness

The repository harness is intentionally not part of the default test run. Against only the checked-in HA Compose project it proves two ready identities, absence of the environment HealthMonitor on both replicas, activation of shared Task queue/reconciler loops, REST distribution, an actual `429` from the single-ingress sensitive-route limiter, stateless MCP initialization plus stateful rejection, polling upgrade affinity, authorized Feathers and native-room delivery with a bounded duplicate-observation window, anonymous denial, the split Codex contract (auth-file import admitted, device polling blocked), authenticated daemon reconnect/restart, and Redis readiness/liveness loss and recovery. Cleanup runs in `finally`, restores stopped branch services, and removes any created API key or board after both success and assertion failure.
