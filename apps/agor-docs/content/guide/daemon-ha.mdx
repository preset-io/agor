...
HA Codex device sign-in additionally requires Linux `flock` (provided by the
`util-linux` package). The official image includes it. Without it, the auth
operation fails closed rather than using an age-stealable filesystem lock.
...
stack declares shared branch/base-repository storage and `persistent-per-user` home storage:
