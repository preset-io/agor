...
config/state. Local Codex subscription auth and OpenCode are refused on these
branches because their credential files cannot currently be separated
safely from the writable config home. A delegated substrate owns any equivalent
credential overlay. The tree needs the **same shared-storage and
