...
Do not substitute the standalone built-in `simple` executor's Codex namespaces for this HA
contract. Those namespaces only separate trusted-local state under one daemon uid; they provide
neither filesystem isolation nor the exact-user storage and locking guarantees required here.
