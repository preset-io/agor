...
must be at least five seconds longer than both the HTTP timeout and renewal scan interval. Storage guarantees
...
retains and renews its PostgreSQL lease; after owner death, another replica can take over only after
database-time expiry. Every result commit locks and rereads the branch and rejects an expired or
replaced token, changed lifecycle generation, inactive status, or archived row. No database
transaction remains open during the HTTP request.
...
2. Apply all migrations before admitting new-version daemons. Migration `0077_environment_health_ha` adds branch observation leases/generations and the narrow discovery policy. Use an all-at-once cohort replacement; old daemons do not understand the new capability contract.
