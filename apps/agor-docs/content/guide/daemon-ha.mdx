...
- Native cross-replica packets in this profile are `cursor-moved`, `cursor-left`, `presence-updated`, and tenant-scoped `repo:cloneError`. `server-info` is socket-local. OAuth packets are behind unsupported HA flows; terminal PTY traffic is disabled and must never enter Redis.
