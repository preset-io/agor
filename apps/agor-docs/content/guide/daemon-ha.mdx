...
HA Codex credential operations additionally require Linux `flock` (provided by
...
has not been verified. `local-only` (and omission) keeps HA Codex credential
...
later Codex Task agree on `.codex/auth.json`. Generation-fenced HA mutations run in the
authority-owning daemon rather than a detached credential writer. Device-code attempts use
PostgreSQL ownership leases; Redis never carries their codes or credentials.
...
2. Apply all migrations before admitting new-version daemons. Migration `0077_environment_health_ha` adds branch observation leases/generations and the narrow discovery policy. Migration `0078_mcp_oauth_pending_flows` replaces legacy MCP OAuth grant/callback authority, migration `0082_github_install_state` moves one-time GitHub setup state into PostgreSQL, and migration `0091_codex_device_auth_attempts` adds Codex poll leases plus completion generations. These protocol migrations require `agor db migrate --yes --offline-cutover` on an existing database. Use an all-at-once cohort replacement; an old Codex daemon has neither attempt nor credential-file generation fencing, and mixed relay revisions intentionally do not exchange events.
