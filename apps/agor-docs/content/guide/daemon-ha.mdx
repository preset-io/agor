...
- Native cross-replica packets in this profile are `cursor-moved`, `cursor-left`, `presence-updated`, tenant-scoped `repo:cloneError`, and tenant/branch terminal cleanup metadata. `server-info` is socket-local. Supported Codex/Claude auth flows use PostgreSQL and direct responses, never Redis OAuth packets. Terminal input/output is always socket-local and must never enter Redis.
...
| Codex device auth                                                              | Supported only with a local exact tenant/user credential route: HA requires `unix_user_mode: sandbox`, `user_home: persistent-per-user`, and the verified `user_home_locking: cross-replica-flock` assertion; shared homes, local-only locks, `filesystem_home` overrides, and delegated homes remain gated. PostgreSQL stores sealed tenant/user-bound attempts, grants one DB-clock poll lease at a time, fences the one-shot exchange and credential generation, and permits poll takeover after owner loss. Status may route to any replica; sticky sessions are not required. The authority-owning daemon performs the generation-fenced file mutation itself, while a non-age-stealable Linux kernel lock prevents a retry from overtaking a still-live writer after database authority loss. Daemon death releases that lock. Persistence is not an exactly-once crash-recovery protocol: ambiguous exchange/write outcomes are not replayed automatically, and the UI lets the user start over immediately. Native-auth resolution rejects HA home overrides and compares the Task creator's credential home with the Session owner's runtime home. SQLite retains its simple process-local standalone flow. |
...
| Claude OAuth/logout/native auth                                                | Supported only on the same local exact tenant/user sandbox route: `persistent-per-user`, `cross-replica-flock`, no `filesystem_home` override, and stable `AGOR_MASTER_SECRET` on every replica. PostgreSQL owns the attempt id, SHA-256 state fingerprint, AES-GCM sealed PKCE/route material, one-shot exchange claim, and monotonic mutation generation under forced tenant RLS. OAuth finalization, logout, replacement starts, and external Claude method/API-key/token patches share the tenant/user advisory lock and file tombstone. The authority-owning daemon writes and validates the exact `.claude/.credentials.json`; stale/ambiguous losers are never replayed or path-deleted. A status or paste-back submit may land on either replica. SQLite retains the process-local standalone flow.                                                                                                                                                                                                                                                                                                                                                                                                          |
