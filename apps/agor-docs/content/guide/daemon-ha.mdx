...
To roll back the binaries, stop the HA cohort, remove all but one daemon from ingress, set `deployment.mode: standalone`, and start only an old-version daemon against PostgreSQL. The additive `0077` and `0078` schema may remain: the old daemon ignores PostgreSQL GitHub install-state hashes and resumes its former per-process setup flow. Those hash rows contain no raw bearer but remain inert until a new-version daemon resumes expiry cleanup, a new-version tenant deletion removes them, or an operator deliberately removes the table. Older tenant-deletion code refuses to delete while the database migration watermark is ahead of that binary.

A schema rollback is a separate, deliberate operation after every new daemon is stopped. Reverse `0078` by dropping the additive `github_install_states` table and its policies; reverse `0077` by dropping the discovery policy/index and seven coordination columns. `REDIS_URL` may stay present because standalone ignores it. Do not rotate shared secrets or switch database engines during an emergency rollback.
