...
| MCP OAuth/callback and OAuth-capable MCP discovery, Claude subscription sign-in/logout, Codex device auth, OpenCode OAuth/native auth | Blocked in the constrained profile. MCP OAuth and Claude sign-in have durable PostgreSQL attempt prerequisites but remain gated pending separate activation. Claude's final credential write/logout still needs the shared generation-fenced credential mutation coordinator; durable attempt ownership alone does not serialize filesystem writers. Codex device attempts still live in one daemon's timer/Map. |
