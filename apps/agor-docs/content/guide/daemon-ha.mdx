...
config/state (and, for Codex subscription auth, a bind onto the caller's real
`auth.json`). It needs the **same shared-storage and `cross-replica-flock`
guarantee documented above for `homes/`**: a prompt or Task dispatched to
replica B must see exactly what replica A wrote, and concurrent writers within
one branch home rely on the same cross-client `flock` semantics. Agor does not
add a separate `executor_storage` key for it — place `branch-homes/` on the same
backing store as `homes/`/`worktrees/`, at the same absolute path on every
replica.
