...
branch configuration. Local Codex subscription auth and OpenCode are refused
for branch-scoped Sessions because their credential files cannot currently be
separated safely from the writable config home. A delegated substrate owns any
equivalent credential overlay. The tree needs the **same shared-storage and
