...
for its Tasks. Short-lived auth-file helpers point to the same store directly, so either daemon
and the later Codex Task agree on `.codex/auth.json`. Device-code attempts use PostgreSQL ownership
leases and generation-fenced writes; Redis never carries their codes or credentials.
...
| Codex device auth                                                                                | Supported only with a locally containable exact tenant/user credential route: HA requires `unix_user_mode: sandbox` plus `user_home: persistent-per-user`. Shared user homes and delegated homes remain gated; although delegated storage can be declared exact-user, Agor cannot prove that a timed-out remote credential writer is gone before releasing its database fence. PostgreSQL stores sealed tenant/user-bound attempts, grants one DB-clock poll lease at a time, fences the one-shot exchange and credential generation, and permits takeover after owner loss. Status may route to any replica; sticky sessions are not required. Executor native-auth resolution compares the Task creator's credential home with the Session owner's runtime home and refuses a mismatch. SQLite retains its simple process-local standalone flow. |
