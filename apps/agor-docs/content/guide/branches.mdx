...
- **Delete permanently**: Irreversibly removes branch metadata and its sessions and conversations through the current metadata-first path. This is not proof of complete storage or external-resource erasure; see the limitations below.
...
The current deletion response and disappearance of a branch card are **not proof of
verified removal of all filesystem and descendant resources**. Do not use them as an
erasure receipt. Internal deletion checkpoints do not by themselves provide a public
deletion-status API or guarantee that every deletion entry point is durable.
