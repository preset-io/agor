...
> **Execution availability:** The settings and executor implementation are being
> integrated with shared branch maintenance admission. Until that integration is
> complete, archive Clean requests are rejected before metadata or filesystem
> changes, even for enabled repositories. Enabling policy alone does not enable
> execution. Preserve remains available. Closing a terminal tab or restarting a
> daemon does not prove that a shell stopped; uncertain containment must not be
