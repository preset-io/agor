...
the prompt is attributed to and executes as the actual caller. Sessions that
use their owner's execution home—including Sessions created before a branch
adopts a branch SDK home—retain the stricter personal-sharing rule. Prompting
one requires that owner to grant personal session sharing and a workspace
administrator to enable the feature. Because continuing that native
conversation uses the owner's home, read [the session-sharing
FAQ](/faq#session-sharing-home-access) before enabling it.
