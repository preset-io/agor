...
when no local branch is retained. Clone recovery requires an actual local `.git`
...
Clean removes ignored build outputs, not the working directory; Delete removes the
...
a teammate intentionally, a branch Manager can choose **Retire teammate** in its
Teammate settings, or `POST /branches/:id/retire-teammate` with `{}`. This atomically
