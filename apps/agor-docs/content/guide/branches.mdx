...
source can still be valuable. There is no undo. Custom commands run relative to
the workspace and do not inherit those preservation guarantees. Checkout scripts
are mutable executable code; never put secrets in cleanup commands.
...
errors. `last_cleanup_succeeded_at` records command success, not bytes reclaimed.
...
are rejected, not dispatched again. Custom commands are bounded to five minutes;
