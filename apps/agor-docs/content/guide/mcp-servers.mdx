...
Catalog's pre-connect readiness is advisory from catalog and saved-connection data; it does not probe the endpoint. Agor checks the live endpoint only when you choose **Connect** and before installing it. Entries may be:
...
Catalog offers the normal **Connect** / **Sign in** flow for all of these providers,
subject to the existing workspace, tenant, server, and branch permissions. Discovery
or a generated client ID does not prove that a provider permits access or accepts
the callback. Provider rejections surface after an attempted connection with safe
recovery guidance; removing a Catalog gate does not resolve external approval requirements.
No provider registration is made until a user starts the normal connection flow.
Existing grants and configured OAuth security policies are unchanged.
...
retire a stored client. Use the safe failure evidence to establish whether a
provider-specific protocol change or provider/admin configuration is needed.
