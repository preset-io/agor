...
The scheduled `MCP catalog health` workflow repeats a read-only initialize/auth probe and OAuth metadata/client-readiness audit. It writes structured warnings and a job summary but does not fail ordinary pull-request CI for third-party downtime. Unit tests inject the network and OAuth discovery seams and never depend on public endpoints.
