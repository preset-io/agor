...
Deleted servers can remain in saved branch/user default lists. Review those
settings to remove stale entries. Agor does not automatically prune these JSON
lists: pruning the last branch entry would change the list to empty and thereby
activate user-default inheritance. Creation-time handling also covers existing
stale lists and a server deleted while a session is being created.

## Catalog visibility

Curators can set `hidden: true` on an entry in
`packages/core/src/mcp-catalog/curated.yaml` to remove it from catalog discovery,
search, and new catalog installs by name. Hidden names return
not found on catalog lookup, readiness, connect, and catalog start-session routes;
there is no request parameter to reveal them. Omit `hidden` or set it to `false`
and redeploy to offer the entry again. All definitions remain validated.

This is not a runtime disable or authorization revocation. Saved connections,
grants, attachment, and reconnect through **My Servers** (or an OAuth widget
naming the saved server ID) retain their existing security and canonical catalog
policy checks. Full definitions remain available internally for those checks and
maintenance, even while hidden.

Vercel, Intercom, Square, Canva, Dropbox, and New Relic are currently hidden based
on the September 9–10 review, not a new provider certification. The first four had
documented provider-client or callback approval requirements. Dropbox involved
trusted-client DCR admission; its observed nonspecific `403` did not prove
callback rejection. New Relic DCR succeeded, but subsequent authorization failed;
allowlisting was not confirmed. Figma is also hidden following a September 24
user report that provider approval is required; this was not independently
verified. There are seven hidden providers. Asana, incident.io, and Datadog are
not hidden by this change. Onboarding recommendations remain hardcoded; Figma
has been removed from the “Build me an app” recommendations so the existing
eligible choices fill the kit. None of the seven hidden providers is actively
recommended; recommendations are not dynamically filtered by catalog visibility.
