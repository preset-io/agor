...
material. It is **not synchronous provider-side revocation**. Already-admitted
mediated requests may finish; new mediated hops and fresh authentication-status
reads no longer accept the removed grant. Direct provider connections in `off`
or `observe` mode remain subject to the provider's own lifetime/revocation rules.
...
1. Stop **all** daemons against the database and close outstanding sign-in windows.
2. Back up the database, then run `agor db migrate --offline-cutover` with the new binary.
3. Start only the new binaries and have a current administrator authorize each
   shared OAuth server again.
...
prefer a forward fix and fresh consent.
