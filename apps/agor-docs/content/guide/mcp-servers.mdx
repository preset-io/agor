...
Settings keeps transport, scope, configured **Enabled/Disabled** state, ownership,
and authentication/discovered capabilities beside each server name. **Shared**
means no private owner; it does not identify who added the server. **View details**
shows creation/update timestamps. Creator and last-modifier identities are not
recorded, so ownership must not be interpreted as an audit trail.

**Not signed in** needs authentication. A discovered tool count is saved capability
information, not a live connectivity check; use the editor's connection test when
