...
[connection diagnostics and mode restrictions](/guide/config-yaml#mcp-connection-security).
...
Set `hidden: true` to withdraw an entry from discovery and new installs by name;
omit it or set it to `false` and redeploy to offer the entry again. Hidden
entries remain validated and retain the policy checks needed by saved connections.
Hiding is not credential revocation.
