...
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| **`sqlite`** _(default)_ | Standalone source-mode dev with SQLite, no branch RBAC, and no Agor executor sandbox. Fastest to boot.                                                                                                                                         | daemon: `3000 + branch.unique_id`, UI: `5000 + …` |
...
| **`sandbox-peruser`**    | SQLite plus `execution.unix_user_mode: sandbox`, which forces branch RBAC, a per-user home, and fail-closed bubblewrap isolation. It does not impersonate host users.                                                                          | same as `sqlite`                                  |
| **`rich`**               | Standalone PostgreSQL + branch RBAC + fail-closed per-user sandbox profile. Also creates Alice/Bob RBAC fixtures. HA remains a separate variant.                                                                                               | same as `sqlite`                                  |
