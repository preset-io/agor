...
| **`ha`**                 | Constrained active-active smoke topology: PostgreSQL, ephemeral Redis, two production-source daemon processes, and nginx affinity. Static tenant and RBAC off; intended for audited HA behavior, not security. | ingress: `3000 + branch.unique_id`                |
...
4. **Pick a variant** in the branch's environment settings (`sqlite` for most things, `postgres` for DB-only parity, `rich` for PostgreSQL + RBAC + per-user sandbox testing, `ha` for HA behavior, or `docs` for documentation work).
