...
deployment-wide password profile. It requires the checked-in development gate
and `NODE_ENV=development`; every newly created or changed
password still follows the secure daemon policy. An existing development
`admin` password remains usable until you change it, after which it cannot be
assigned again through normal user APIs.
