...
| **`postgres`**           | `sqlite` topology with PostgreSQL schema parity. Application RBAC is always on; the Agor executor sandbox remains off. **Requires Postgres 13+** (migrations use `gen_random_uuid()` from `pg_catalog`).                                       | same as `sqlite`                                  |
| **`sandbox`**            | SQLite plus the tunable, shared-home bubblewrap filesystem sandbox. Application RBAC is always on; sandbox unavailability is not fail-closed.                                                                                                  | same as `sqlite`                                  |
...
| **`rich`**               | Standalone PostgreSQL + fail-closed per-user sandbox profile. Includes Alice/Bob fixtures, branch SDK homes, local S3, and cold storage for new standalone clones. HA remains separate.                                                                                                             | same as `sqlite`                                  |
...
### Cold storage in rich

Use **rich** for cold-storage testing. SQLite and plain Postgres remain lightweight.

The rich Compose overlay enables `execution.branch_storage.cold_storage_enabled`,
defaults new branches to `clone`, and sends uploads and durable bundles through
the existing AWS SDK client to [S3Mock](https://github.com/adobe/S3Mock) 5.2.0.
The emulator creates bucket `agor-dev-storage` automatically, uses dummy
credentials, and exposes no host port. It is a development-only, unauthenticated
service, not a production storage backend or an IAM test. Tenant/branch object
namespacing and Agor authorization remain unchanged.

These process overrides apply on startup even to an existing dev config, without
rewriting `config.yaml`:

| Environment variable | Rich value |
| --- | --- |
...
| `AGOR_S3MOCK_CHECKSUM_COMPATIBILITY` | `true` |

`AGOR_S3_FORCE_PATH_STYLE` selects path-style SDK requests. The emulator-only
checksum compatibility switch removes the redundant SDK algorithm hint when
an actual checksum header is present: S3Mock 5.2.0 otherwise ignores that checksum.
SDK hashing, provider verification, and receipt checks remain enabled; never
enable this workaround for production S3. Normal S3 addressing stays unchanged
when unset. Outside this overlay, cooling is still default-off. Existing Git
worktrees are not converted, and existing local uploads are not migrated into S3.

After updating source, re-render/select rich and recreate the environment when
ready so Compose picks up the dependency and variables. Do not merely restart the
daemon. Create a **disposable clone**, add staged/dirty/untracked/ignored files,
stop its tasks, terminals and environment, then cool and restore it. Compare Git
status, contents, executable permissions and symlinks afterward; also prompt a
cold clone to exercise queued restore. Check the storage stage/error if cooling
fails: provider checksum checks are never disabled for the emulator.

Object files are retained across stop/start in the project-scoped `s3mock-data`
volume. S3Mock does not guarantee persisted-data compatibility; test restore after
a restart before relying on this fixture, and never keep important work solely
in it.
**Nuke deletes the database and object volume together**; use disposable data only.
Do not delete that volume alone or switch storage endpoints while branches are
cold—the database receipt points to their sole workspace copy. A successful
emulator test is not proof of real AWS behavior.

An opt-in Vitest test exercises small uploads, checksum rejection, and a real
dirty clone through multipart upload and staged restore against a running emulator.
Inside the rich container (no service startup performed by the test):

```bash
AGOR_TEST_S3MOCK_ENDPOINT=http://s3mock:9090 pnpm --filter @agor/daemon exec vitest run src/utils/s3mock-bundle.integration.test.ts
```

It uses dummy credentials and removes only its disposable object prefix and clone.
Recreate rich after adding the checksum compatibility environment variable; a
daemon watch restart alone does not update container environment variables.

A representative variant block, abridged from the file:

```yaml
sqlite:
  description: Single-user dev with SQLite. Always-on RBAC, no executor sandbox. Fastest to boot.
  start: >-
