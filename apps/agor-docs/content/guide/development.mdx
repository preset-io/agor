...
Pick a variant by setting `AGOR_VARIANT` in your environment or selecting it in the branch's environment settings. You can also render it manually with the appropriate `docker compose` invocation from the file.
...
- Fresh isolated clones may still need generated declarations; root Turbo validation prepares required dependency outputs in graph order
...
- Builds required dependency outputs before typechecking dependents
...
- Do not run `pnpm build` merely to refresh the active watch checkout; one-shot repository-native validation remains appropriate
- In fresh isolated clones, use root Turbo validation or the smallest prerequisite package build when missing workspace declarations block a check
