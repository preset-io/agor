...
### Cold storage in rich / full

Use **rich** for cold-storage testing. **full** is the same configuration via
`extends: rich`, retained only so existing selections keep working; there is no
second full stack to maintain. SQLite and plain Postgres remain lightweight.
...
The latter selects path-style SDK requests; normal S3 addressing stays unchanged
