...
Select a registered repository you control, or expand **Add a repository** and
paste its URL. Agor registers/clones that repository and detects its default
...
a custom source URL/branch, or the destination’s own source branch.
...
GitHub repository sign-in uses your existing encrypted user-credential service;
it is separate from GitHub MCP sign-in. Errors and clone retries stay inline.
...
already created, restore its original destination to resume it.
