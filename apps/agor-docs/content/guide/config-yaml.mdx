...
delegated, simple, shared-home, or disabled-sandbox execution. HA additionally
requires durable attempts and cross-replica home locking; only the contained
exact-user sandbox topology satisfies all three predicates. The daemon keeps
the refreshable grant in the canonical per-user
`~/.claude/.credentials.json`, refreshes it before task launch, and sends the
runtime only the current access token. The sandbox masks that one canonical
file from Claude while preserving the rest of `~/.claude` for settings and
fork/resume state; Codex native auth is not masked by this change.
