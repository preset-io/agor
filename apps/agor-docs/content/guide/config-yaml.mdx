...
Topology gates still apply after authorization; the constrained HA profile in
this release exposes Claude OAuth only when its resolved deployment capability
also proves durable attempts, an exact per-user credential home, and
cross-replica home locking. Other HA topologies remain fail-closed even when
the flag is true.
