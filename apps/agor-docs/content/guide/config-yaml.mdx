...
ui:
  environment_notice:
    severity: warning # info (default) | success | warning | error
    title: Remote environment availability
    message: >-
      This Agor instance can call configured environment webhooks, but it does
      not host development environments. Connect a supported runtime provider.
    link:
      label: Read the environment guide
      url: https://agor.live/guide/environment-configuration
```

`title` and `message` are required plain text (120 and 2,000 characters
maximum). Markdown and HTML are not interpreted. `link` is optional, but when
present both `label` (120 characters maximum) and `url` (2,048 characters
maximum) are required. URLs must be HTTPS or a same-origin path beginning with
a single `/`; URL credentials and protocol-relative links are rejected.
...
The notice is deliberately not dismissible: it communicates deployment policy,
and dismissal would require user-scoped persistence plus notice versioning to
avoid hiding changed policy. Omit `ui.environment_notice` to preserve the
