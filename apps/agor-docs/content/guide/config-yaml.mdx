...
auth helper, and does not claim successful provider validation.

Access is refreshed before SDK launch, not inside an already-running SDK. A long
task or permission wait can outlive its token. After authentication expiry, use
**Continue** to create a new task with a fresh token and existing SDK resume state.
...
Migration `0109_user_provider_oauth_grants` ships for both database dialects and
