...
Daemon-driven **Sign in with Claude** is enabled by default on supported execution
topologies, including when `agentic_tools.claude_subscription_oauth` is omitted.
Set it to `false` to opt out. API-key setup, manually pasted `claude setup-token`
credentials, ordinary Claude sessions, and disconnect/cleanup remain available
regardless of this setting.

```yaml
agentic_tools:
  claude_subscription_oauth: true # Default; set false to disable sign-in
...
After changing this setting, restart every daemon replica with the same immutable
config. When disabled, `/claude-auth/oauth` fails with
`CLAUDE_SUBSCRIPTION_OAUTH_DISABLED` and Settings and onboarding omit the OAuth
option. Operators remain responsible for appropriate provider authorization and
terms; enabling the default does not establish a provider/client contract.
Topology and authentication gates still apply. This release requires the exact
