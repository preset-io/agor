...
at least 15 Unicode characters, rejects values above bcrypt's 72-byte UTF-8 boundary, and rejects a
local list of common or account-specific values. It deliberately has no uppercase/digit/symbol
composition rules, periodic rotation rule, or paste restriction, so password managers and pasted
passphrases work normally. Candidate passwords are never sent to an external breach-checking
service.
...
password, but the next assignment must pass `secure`. An accepted password change invalidates
existing browser access and refresh tokens; it does not revoke separately managed API keys.
...
candidate or the deny-list. `/health.auth.passwordPolicy` exposes the same safe requirements for
form help; server validation remains authoritative.
...
`LOCAL_AUTH_DISABLED` as appropriate.

For rollout, add or retain `identity.password_policy: secure`, upgrade every daemon, then verify
the effective snapshot with `agor config --yaml`. No database migration or forced user reset is
required. For rollback to a version that predates this key, first remove the additive key from
YAML after the old binary is ready to take over; rollback restores the older binary's weaker
assignment behavior, so keep the window short and restrict password-write access during it.
