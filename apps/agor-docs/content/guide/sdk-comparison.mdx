...
The model selector uses a protected read-only adapter. Each discovery starts a
fresh managed OpenCode `1.14.33` server in the authenticated caller's credential
namespace, optionally scopes it to a server-authorized branch directory, reads
`config.providers()`, `config.get()`, and native runtime availability, returns
only provider/model display metadata and availability, then stops the server.
Agor keeps no provider registry, model cache, server pool, or persistent
OpenCode server.

Task and catalog servers for the same subject may run concurrently against
...
session owner's personal Agor default. If none supplies a complete pair, Agor
returns an actionable error instead of delegating to an OpenCode default. A
provider's suggested model and a project-configured pair are selection context,
not an automatic default. If a stored pair later disappears, a refresh marks it
unavailable without replacing it; it remains stored and visible until the user
replaces or edits it. The same task-scoped server rejects an unavailable exact
pair before OpenCode session creation, prompt submission, or model-provider
traffic. A configured catalog is not proof that a remote provider will accept a
request.
