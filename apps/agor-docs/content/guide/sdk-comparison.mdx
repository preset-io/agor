...
For agent selection, start with [Choosing an Agent](#choosing-an-agent). The
reference below also covers integration behavior and SDK-specific limitations.
Connection setup is covered separately in [MCP Catalog & Connections](/guide/mcp-servers).
...
| **Permission requests**     | ✅ Rich         | ⚠️ Basic           | ⚠️ Basic         | ✅ Interactive  | ✅ Interactive     | OpenCode routes permission decisions through Agor and honors session permission mode          |
...
Agor prefers the provider's context-usage snapshot when one is available. Claude
and Codex can report this directly; availability and fallback estimates depend
on the integration and the data returned for that turn. Missing usage is not
evidence that the context window is empty.

Use the meter to judge when to compact or start a focused session, not as a
guarantee of remaining capacity. Cumulative token spend is not current context
occupancy: repeated cached input can be billed across multiple tool calls.
