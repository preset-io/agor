...
Gemini is **Beta** and supports **AI Studio API keys only**. Add a key in
Settings → Gemini. Google-account sign-in and Vertex AI are not supported.

- **Accept edits** (the default): auto-approves reads, edits and permitted MCP tools;
  shell, background processes and web access need **Bypass permissions**.
- **Bypass permissions** uses the SDK's YOLO behavior for built-ins, shell, web,
  skills and loaded sub-agents. It is not a sandbox. Agor's MCP selection and
  per-tool restrictions still apply.
- Manual approval, interactive questions and plan mode are unavailable. Existing
  Manual configurations fail when run; switch to Accept edits or Bypass. If a
  workspace manages your preset, ask an admin to change it.
- Built-in file edits can update build files inside the branch. MCP tools and
  sub-agents cannot edit build files. An agent declaring an MCP server name that
  clashes with an Agor-configured server cannot run. Two loaded agents declaring
  the same MCP server name share one merged server configuration in Bypass.
- Attachments and thinking display are not supported. Hooks and extensions are off.
- Follow-ups use SDK conversation recordings in the session's SDK home, including
  across executors. If history cannot be restored, a notice explains the fresh start.
- The default model is `gemini-3.8-flash`. The picker also offers `gemini-3.7-flash`,
  ...
  are remapped with a notice; retired Pro selections require another model.
- Usage sums all model turns; context size reflects the last turn. Costs are estimates.
...
| **Permission requests**     | ✅ Rich         | ⚠️ Basic           | ❌ No Manual | ✅ Interactive  | ✅ Interactive     | OpenCode routes permission decisions through Agor and honors session permission mode          |
...
| **Tool execution**          | ✅ Rich widgets | ⚠️ Basic           | ✅ Standard results | ✅ Rich events  | ✅ Rich events     | OpenCode streams tool parts with state tracking (pending/running/completed)                   |
| **Session continuity**      | ✅ SDK-managed  | ✅ History-based   | ✅ SDK-managed | ✅ SDK-managed  | ✅ SDK-managed     | Gemini resumes native SDK conversation recordings |
| **Token usage tracking**    | ✅ Full         | ✅ Full            | ✅ Per-turn | ⚠️ Via events   | ✅ Via step-finish | OpenCode provides token counts and cost via `step-finish` message parts                       |
| **Context window tracking** | 🟡 Estimated    | 🟡 Estimated       | 🟡 Estimated | 🟡 Estimated    | 🟡 Estimated       | Agor derives cumulative conversation usage from SDK metrics                                   |
...
- Progress, compression and blocked-action notices
...
**Gemini:** **Task cancellation**

- Stop aborts streams and running tools and marks the task stopped
- Follow-ups resume the SDK recording; side effects already performed are not rolled back
...
- Agor's built-in MCP server auto-configured with the session token in the
...
**Gemini:** **Accept edits and Bypass**

See [Gemini Beta](#gemini-beta) for the permission boundary and limitations.
...
- Agor routes decisions through the same task-scoped permission service used by other agents
...
**Agor Implementation:** Permission modals adapt based on agent type. Claude shows persistent scope options (Project/User/Local); Copilot and OpenCode use task-scoped approve/deny decisions.
...
**Gemini:** **Standard tool results**

- Google function calls and results map to Agor tool blocks
- Reads, edits, shell and searches use the standard renderers
- Denials and tool errors remain visible in the transcript
...
**Gemini:** **SDK-managed conversation recordings**

- Restores the exact session recording through SDK resume
- Missing or ambiguous history continues fresh with a notice
...
Gemini resumes its SDK recording rather than reconstructing history from rendered transcript messages.
...
**Gemini:** **Per-turn usage**

- Sums input, output and cache-read usage across model turns
- Uses the last turn for context size; costs are estimated
...
**Agor Implementation:** Token data stored in `messages` table, aggregated for session-level analytics. Claude, Codex, and OpenCode sessions display token/context pills today; Gemini also records per-turn usage; Copilot support varies by event.
...
- Context size uses the latest model turn, not summed task usage
- Task usage sums all turns; costs are estimated
...
- **Server:** Agor starts and contains a task-scoped `opencode serve` process
...
OpenCode supports 75+ providers. Connect them in Settings → OpenCode Providers; Agor keeps native credentials in a tenant-and-user-scoped home and does not copy provider secrets into the executor environment.
...
- **Remote (Streamable HTTP):** Agor injects its built-in MCP server URL with
...
- **Execution is local-only.** Agor must be able to contain the native writer process safely.
...
- **Token tracking for Copilot** - Extract usage data from GitHub APIs
