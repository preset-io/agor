...
Provider settings remain dynamic because OpenCode owns its native API-key and
OAuth methods. Model defaults do not start a server: Agor combines a model
catalog versioned with the packaged OpenCode runtime with a protected,
server-free read of the caller's saved provider IDs. Known choices render
immediately, and the identity-scoped availability read is shared until an
authentication mutation invalidates it. Normal selectors show only configured
providers plus credentialless OpenCode Zen. A configured provider outside the
known catalog can still be selected by entering an exact model ID manually;
the task runtime validates every pair against the caller's branch configuration
and credentials.
...
session creation, Agor uses the first known provider with saved credentials, or
the first catalog provider that needs no saved credential. That create-time
fallback is persisted and never replaces an explicit or existing selection. If
a stored pair later falls outside the known catalog or loses credential
evidence, it remains stored and visible until the user replaces or edits it.
The same task-scoped server rejects an unavailable exact pair before OpenCode session
creation, prompt submission, or model-provider traffic. Catalog availability is
