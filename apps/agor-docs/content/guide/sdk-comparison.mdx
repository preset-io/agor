...
cancellation, and verifies saved-credential presence with a fresh discovery.
...
The model selector uses a protected read-only adapter and an Agor-curated list
of common OpenCode provider/model pairs. After validating the caller's private
native-data boundary, the read inspects saved provider IDs so configured known
providers can be suggested, but it does not start an OpenCode server. OpenCode
Zen is available without a saved API key. Providers or models outside the
curated list can still be entered as an exact pair; the task runtime validates
that pair against the caller's branch configuration and credentials.
...
session creation, Agor uses the first curated provider with saved credentials,
or the first curated provider available without credentials. That create-time
fallback is persisted and never replaces an explicit or existing selection. If
a stored pair later falls outside the known list or loses credential evidence,
it remains stored and visible until the user replaces or edits it. The same
