...
| **Permission requests**     | ✅ Rich         | ⚠️ Basic           | ⚠️ Basic         | ✅ Interactive  | ✅ Interactive    | OpenCode routes permission events through Agor and supports remembered stable patterns        |
...
| **Token usage tracking**    | ✅ Full         | ✅ Full            | ❌ No            | ⚠️ Via events   | ⚠️ Captured       | OpenCode cost, token, and cache metrics are retained from `step-finish` parts                 |
| **Context window tracking** | 🟡 Estimated    | 🟡 Estimated       | ❌ No            | 🟡 Estimated    | ❌ Not aggregated | OpenCode per-step metrics are not yet converted into a context-window occupancy snapshot      |
...
- MCP servers are included in the managed runtime configuration before startup
...
- User-defined MCP servers are resolved per invocation from the session's attachments
...
- OpenCode emits permission events when tool execution requires approval
- Agor applies explicit invocation-level `ask` rules for every packaged OpenCode permission category; permissive repository rules cannot bypass the Agor decision path
- Agor uses the session permission mode and shows its existing approve/deny UI when required
- One-time and automatic approvals reply with `'once'`; applicable remembered approvals reply with `'always'`
- Denial, timeout, and cancellation reply with `'reject'`
- Permissions include `external_directory` access when working outside the default project

**Agor Implementation:** Permission modals adapt based on agent type. Claude and OpenCode can apply remembered scopes when the provider can represent them; Copilot shows approve/deny only.
...
- Agor renders per-session project context to a temp file and passes it via the SDK's `model_instructions_file` config, never mutating the user's `~/.codex/` directory
- No manual injection needed
...
- Agor's invocation-scoped permission policy remains authoritative over repository permission settings
...
- Agor stores `sdk_session_id` before submitting the first prompt
- Each later task starts a new server and resumes that ID from OpenCode state under the same executor identity
- A missing or invalid stored session fails with an actionable error; Agor does not silently create a replacement
- No Agor message replay is needed
...
- Agor sums all new assistant `step-finish` parts and stores the result in message metadata under the `opencode` key
- These metrics are retained for inspection; they are not yet normalized into TaskHeader token/context pills or session context occupancy

**Agor Implementation:** Claude and Codex feed the normalized task-level usage pipeline today. OpenCode retains authoritative per-turn metrics in message metadata; Gemini and Copilot are not yet wired into task-level usage.
...
- **Server:** Agor starts one authenticated loopback server for each executor task and stops it before the task returns
- **Runtime:** Agor supplies the exact matching OpenCode SDK and native CLI from its package; no global OpenCode installation or separately managed server is used
- **SDK:** `@opencode-ai/sdk` communicates with that task-scoped server
...
Agor's per-task Basic credential authenticates only the private SDK-to-server connection. It is not a model-provider credential. The executor identity must still have the selected provider configured through its injected environment or OpenCode credential state:

- In `strict` Unix mode, OpenCode reads state as the session creator's Unix user.
- In `insulated` mode, it reads state as the configured executor user.
...
Agor places OpenCode's XDG data, configuration, cache, and state roots beneath
the authenticated tenant-and-user namespace. This provides OS-enforced
separation only when users execute under distinct Unix identities; same-UID
modes remain logical isolation.
...
cancellation, and returns an updated configuration snapshot before that server
closes.
The settings surface reports runtime availability separately from credential
presence (`present`, `absent`, or `unknown`). Built-in availability, including
Zen, is not proof of a removable credential; removal is offered only when
presence is `present`. Neither state makes a paid or provider-validation
request. Failed or cancelled callbacks leave credential contents under
OpenCode's native ownership rather than using an Agor rollback.

Provider settings and model defaults use the same protected, secret-safe
configuration snapshot. One disposable server reads OpenCode's provider,
authentication, model, and branch configuration APIs; Agor exposes only IDs,
names, availability, authentication methods, credential presence, and model
status. A short-lived browser cache lets the Providers and Defaults tabs reuse
that snapshot instead of starting duplicate discovery servers. OpenCode Zen
does not require a saved API key when OpenCode reports it as available.
Providers or models outside discovery can still be entered as an exact pair;
the task runtime validates that pair against the caller's branch configuration
and credentials.

Multiple task servers for the same subject may run concurrently against
OpenCode's native state. Credential changes and OAuth attempts are serialized,
but are not coordinated with an already-running turn; the change is guaranteed
for servers started after the mutation.

Every runnable OpenCode session stores one exact Agor provider/model pair.
Resolution is session override, then preset or workspace default, then the
session owner's personal Agor default. If none supplies a complete pair during
session creation, Agor uses the first discovered provider with saved
credentials and an active OpenCode default model, or the first available
provider with an active default. That create-time fallback is persisted and
never replaces an explicit or existing selection. If a stored pair later falls
outside discovery or loses credential evidence, it remains stored and visible
until the user replaces or edits it. The same
task-scoped server rejects an unavailable exact pair before OpenCode session
creation, prompt submission, or model-provider traffic. Discovery availability is
not proof that a remote provider will accept a request.

Collaborators configuring a session owned by somebody else do not receive that
owner's private catalog. They can preserve the stored pair or enter exact IDs
manually; task execution still uses the immutable session owner's OpenCode
credentials.

### Session Continuity

The managed server is disposable, but OpenCode session state is durable under the executor identity. Agor validates the stored `sdk_session_id` on every restart. Changing the executor identity, removing its OpenCode data directory, or supplying an invalid stored ID makes resume fail instead of starting an unrelated conversation.

### MCP Integration

OpenCode receives MCP servers in its task-scoped startup configuration:
...
- MCP servers are resolved before the prompt and scoped to the branch directory
- Startup fails before spawning OpenCode when the built-in token, repository resolution, a required template, attachment configuration, or required OAuth hydration is unavailable

### Permissions and Process Containment

- Non-automatic permission requests use Agor's existing permission UI and timeout/cancellation paths.
- Explicit invocation-level rules force interceptable operations through that path even when project configuration says `allow`.
- One-time approval does not create a remembered rule. A remembered approval is applied only when OpenCode supplies a stable pattern that can represent it.
- A denial is delivered to OpenCode before the task becomes terminal, so the session cannot accept a second turn while managed cleanup is still settling.
- Denial, timeout, and Stop reject the pending provider request and run the same bounded cleanup path. Collector shutdown is time-bounded even when an iterator ignores cancellation.
- On Linux and macOS, the server is a non-detached descendant of the executor process group. Local cleanup stops and awaits it; daemon Stop and watchdog containment provide the final group-level backstop.
- Windows group containment is not currently a verified support boundary.

### Startup Failures

Startup errors identify whether the packaged binary is missing, the SDK/CLI versions differ, the server exited, or readiness timed out. The diagnostic includes only a bounded tail of server output. The same turn-local sanitizer covers startup, post-readiness provider/runtime errors, stream-error broadcasts, and final rethrows, redacting the managed server credential, provider credential environment values, MCP tokens and headers, and invocation-config secrets. Reinstall Agor with optional dependencies enabled when the packaged native binary is reported missing.

Completed transcripts are read back from OpenCode rather than trusted from the prompt response. A new assistant turn must contain renderable text, reasoning, or tool content. Reasoning-only output is retained as a thinking block and used as the textual fallback; step-only or empty output fails reconciliation.

### Limitations

- **Session forking** not yet supported (OpenCode doesn't provide fork/replay API)
- **Session import/export** not available
- **Permissions** follow the session's Agor policy, including interactive approval when required
