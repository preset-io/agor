...
prompt. OpenCode's data, configuration, cache, and state XDG roots are all
placed beneath that namespace.
...
- “Available in runtime” and “Saved credential” are separate states. Runtime
  availability includes built-in providers such as Zen and does not imply that
  a removable credential exists.
- Saved-credential presence is `present`, `absent`, or `unknown`. Agor offers
  removal only for `present`; missing or unreadable credential evidence is
  `unknown`, not `absent`.
- Neither state verifies a request with the remote provider.
...
- Native OpenCode credential storage is unavailable when multi-tenancy uses
  ...
  OpenCode.
- Native OpenCode credential storage is also unavailable in `delegated` Unix
  user mode. The execution substrate does not yet provide the daemon with a
  trusted per-user home boundary for native OpenCode state, so authentication,
  saved-provider reads, and task execution reject before accessing OpenCode
  state.
- Task and authentication servers for one user may run concurrently. Credential
  mutations and OAuth attempts are serialized; a mutation is guaranteed for
  later server starts, not an already-running turn. Model selection reads saved
  provider evidence without starting a server.
