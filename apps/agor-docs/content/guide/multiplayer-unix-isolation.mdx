...
subscription auth is refused for local execution because Codex cannot
independently relocate or safely mount its actor-writable `auth.json`. A
reviewed delegated launcher may provide its own caller-scoped credential
overlay.
