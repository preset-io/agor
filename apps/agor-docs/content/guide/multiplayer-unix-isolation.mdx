...
>   sandbox: the daemon trust root and sibling branches are masked, so a member
>   shell cannot read `config.yaml`/`agor.db`/the JWT secret. (Terminals do not
>   yet get the full per-user-home/RBAC-mount treatment that prompts do.)
