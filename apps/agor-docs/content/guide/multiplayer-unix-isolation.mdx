...
# Exact branch-root deletion uses anchored sudo 1.9.10+ argument regexes.
# Install these rules from docker/sudoers/agor-daemon.sudoers; do not replace
# them with /worktrees/* wildcards, which also match slashes and whitespace.
...
- Use sudo 1.9.10 or newer. The branch-delete grants use anchored POSIX
  argument regular expressions; older sudo versions cannot enforce that
  exact-root boundary and must not install those grants.
- Reinstall the shipped sudoers file after upgrading. Strict and insulated
  branch deletion needs its exact-root `rm` rules; without them,
  the branch is retained with a retryable `delete_failed` filesystem status.
  The rules cover both current two-segment repository slugs and supported
  legacy one-segment repository layouts.
- If a daemon or executor exits after reserving `deleting`, repeat the same
  authorized delete. Agor assigns a new operation generation; any prior
  executor becomes stale and cannot publish state or remove metadata. A
  repository retry must keep the original keep-files/delete-files choice while
  that deletion lifecycle remains on the row.
