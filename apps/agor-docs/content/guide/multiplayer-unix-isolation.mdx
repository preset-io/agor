...
The migration removes only the old ownership-immutability triggers; it does not
rewrite existing owners. Authorization, successor eligibility and stale-owner
checks run in the shared application transaction on SQLite and PostgreSQL.
Existing database reference protections remain in place (SQLite owner-update
eligibility is checked by the application, not a replacement trigger).
