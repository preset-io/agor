...
the task runs as the actual caller while native SDK state remains branch-owned.
An **execution-home** Session retains the stricter historical rule: prompting
it requires a workspace administrator to enable personal session sharing and
the Session owner to name trusted people or groups. That path uses the owner's
home to preserve native agent-tool state, so review [the session-sharing
warning](/faq#session-sharing-home-access) first. When the gate is disabled,
Agor and connected gateway channels return an actionable denial rather than
