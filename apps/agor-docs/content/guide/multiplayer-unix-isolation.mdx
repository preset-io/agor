...
- Native OpenCode credential storage is also unavailable in `delegated` Unix
  ...
  catalog discovery, and task execution reject before starting OpenCode.
- Task and catalog servers for one user may run concurrently. Credential
  mutations and OAuth attempts are serialized; a mutation is guaranteed for
  later server starts, not an already-running turn.
