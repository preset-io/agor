...
an **executor filesystem sandbox**: every agent, terminal, and git/file op is
wrapped in [bubblewrap](https://github.com/containers/bubblewrap) at the single
spawn chokepoint. Everything runs as the daemon user, but the **mount namespace**
— computed directly from Agor's RBAC — enforces isolation.
...
It is **filesystem-only** — the network namespace stays shared, so the executor
keeps daemon/model connectivity. Network egress control, if wanted, is deferred
to each tool's own config.
...
`sandbox` mode implies `branch_rbac: true`, `sandbox.enabled: true`,
`sandbox.home_mode: per_user`, and fail-closed (`fail_if_unavailable: true`).
Fine-tune via `execution.sandbox.*` if needed (see the design notes in
`context/explorations/executor-sandboxing.md`). No sudoers, no Unix accounts.
