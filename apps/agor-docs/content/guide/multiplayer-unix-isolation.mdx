...
`per_branch`, the first supported-tool prompt on a branch stickily records the
intent and creates `<tenantDataRoot>/branch-homes/<branchId>`. Disabling the
deployment setting later stops new adoption but does not strand an existing
branch's state. Archiving retains the directory; permanently deleting the
branch removes it.
...
`{branch_sdk_home}`. The branch-home value is shell-escaped and is empty when
the branch has not adopted one; the launcher owns its mount and tool-environment
policy. `{unix_user}` remains temporarily as an opaque, validated home key for
