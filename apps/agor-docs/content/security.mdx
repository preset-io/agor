---
title: Security
description: How to operate Agor safely, covering deployment modes, what's enforced, what's not, and what every admin needs to know before sharing access.
...
App-layer permission checks for branch ownership and access tiers. No OS-level isolation, so all execution still runs as the daemon user.
...
Operators can allow origins for every workspace with `security.cors.origins`.
The configured browser-facing application origin is included automatically.
In list mode, workspace CORS management is enabled by default whenever the
daemon can resolve the workspace before authentication:
...
security:
  cors:
    mode: list
    origins:
      - https://operator-console.example.com
```

Admins can then add or remove exact origins from **Settings → CORS**. Changes
are stored for that workspace and take effect live; they do not require editing
`config.yaml` or restarting the daemon. The page shows only workspace-managed
additions; deployment-level and built-in origins remain operator-side details.

Operators can disable this capability with
`security.cors.tenant_origins.enabled: false`.

Only add websites you trust. Depending on how authentication is configured, an
allowed website may make browser requests using a signed-in user's access.
CORS does not replace authentication, permissions, or tenant isolation.

Hosted multi-tenant deployments must provide trusted tenant-routing context on
browser preflight requests. See
[`context/concepts/security.md`](https://github.com/preset-io/agor/blob/main/context/concepts/security.md)
for the operator-level routing and compatibility details.
...
- **Audit `/var/log/auth.log`** in Unix isolation modes, where sudo activity is the trail.
