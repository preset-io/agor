...
The launcher command receives a curated, secret-free process environment. The authenticated executor payload (including the task's resolved user environment) is delivered over stdin, not argv. Give the launcher its own workload identity in the delegated substrate; do not depend on inheriting daemon database, JWT, master-key, or provider credentials.
...
Per-user API keys and env vars are encrypted at rest and only decrypted when needed. API reads expose names, scope, and presence metadata—not ciphertext or plaintext, including to admins. Admins can replace or delete another lower-role user's stored value where user-management policy permits, but there is no application-level "reveal" operation. **The daemon still hosts the secrets.** A malicious administrator with code/config/host control, or a compromised daemon account, can recover them. Invite administrators you trust with tool credentials.
