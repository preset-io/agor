...
- Enable a completion callback when the child should report back automatically.
- The child’s history is kept: inspect it, prompt it again, fork it, or spawn
  nested subsessions.

Ask your agent to arrange the callback when delegating. See
[completion notifications](#how-do-i-get-notified-when-a-session-on-another-branch-finishes)
for one-off and persistent callbacks.
...
Session sharing applies only to **branch-home Sessions**. A workspace admin
first enables it in **Settings → Workspace → Preferences**; a Board or Branch
Manager then enables **Allow shared session prompting** in branch defaults or
an individual branch override. Collaborators and Managers may then continue
another person's Session.

The shared boundary is the conversation and branch SDK state—not the creator's
home or identity. Every prompt records and executes as its actual caller, using
that person's execution home, managed variables, provider and connector
credentials, and private MCP visibility.

People who can prompt a Session can read its conversation and influence its
future context. That can be useful for handoffs and collaborative debugging,
but it also means prompts and tool results become part of context that later
participants and agents may rely on.

**Execution-home Sessions are never shareable.** This includes historical
Sessions created before a branch adopted branch SDK homes and new Sessions
created while the deployment opts out. Start a new independent branch-home
Session when collaborators need to continue the work.

Both switches default off. Disabling the workspace preference also clears all
board and branch opt-ins, so turning it back on cannot silently reopen old
sharing choices.
...
| **Parent waits?**       | No                                         | No                                            |
| **Reports back?**       | No                                         | Optional completion callback                  |
