...
**Session sharing also shares the session owner's execution home (`~/`).** Agor keeps a shared session attached to its owner's home so the underlying agentic tool can continue its native conversation. Each prompt is still attributed to the person who sent it, and Agor injects that caller's managed environment variables and credentials—not the session owner's.
...
Give your colleague read access to the original session and have them create a new session under their own identity and home. They can ask their agent something like:

> Read the sibling session that implemented the feature, then pick up from where it stopped in this new session.

This preserves access to the prior reasoning without sharing the original execution home. Per-session homes are on the roadmap; until they exist, continuing another person's native agentic-tool session requires sharing that session owner's home.
