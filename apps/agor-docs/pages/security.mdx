...
| Mode | `worktree_rbac` | `unix_user_mode` | Who's it for? |
|---|---|---|---|
...
- ✅ **Authentication on every call** — REST and Socket.IO require a JWT. Anonymous mode (if enabled) is read-only `viewer`.
...
Anyone who can DM the Slack bot (or `@agor`-mention on a wired GitHub repo) can prompt an agent to read, write, and execute code in the configured worktree. Treat enabling the [Message Gateway](/guide/message-gateway) as **giving message senders SSH access to that worktree**.
...
### Codespaces / Public Tunnels

GitHub Codespaces only forwards ports to the Codespace owner by default. The moment those ports are made public, treat the environment as compromised — API keys, repo contents, and in-progress work can leak. If you need shared access, prefer a reverse proxy with auth, or switch to a server with `unix_user_mode: strict`.
