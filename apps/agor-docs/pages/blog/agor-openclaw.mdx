...
description: 'We recreated the OpenClaw agent framework to run entirely within Agor—persistent agents with full visibility, introspection, and multi-agent coordination on a spatial canvas. The results are crazy amazing.'
date: 2026-02-04
---

# Agor-OpenClaw: OpenClaw Patterns Running 100% Inside Agor

**tl;dr:** We took [OpenClaw's](https://openclaw.ai/) brilliant agent loop architecture (SOUL.md, MEMORY.md, persistent identity) and retrofitted it to run 100% inside [Agor](https://agor.live). Everything is visible on a spatial canvas. Multi-agent coordination through session trees. State tracked in git. The first bootstrap went beautifully. This is super promising.

---

## What Happened

Yesterday we published [Agor Cloud](/blog/agor-cloud), announcing our fully managed service. Today we're announcing something equally exciting but completely different: **agor-openclaw**, an adaptation of the OpenClaw agent framework that runs entirely within Agor.

[OpenClaw](https://openclaw.ai/) has shown us what persistent AI agents should look like—continuous identity, layered memory, self-improvement through file modifications, task tracking. It's a complete framework for agent continuity.

But OpenClaw runs as a standalone process. What if we could take those patterns and run them **inside Agor**, gaining:
...
> "Hey, I noticed you're stuck on the type errors. Try using `as any` for now and we'll fix it properly later."
...
Agor-OpenClaw adds visibility, multi-agent coordination, and git-native state tracking.

---

## What's Next

This is day one. We're already seeing crazy promising results:
...
We want agor-openclaw agents to:
...
We're open-sourcing this experiment. The framework is MIT licensed. If you want to:
...
**Join us:** [github.com/mistercrunch/agor-openclaw](https://github.com/mistercrunch/agor-openclaw)
...
The agent loop architecture—SOUL.md, MEMORY.md, persistent identity, self-improvement—that's OpenClaw's innovation. We just adapted it to run inside Agor.
...
OpenClaw proved it for personal automation. We're exploring what it means for developer workflows.
...
OpenClaw figured this out. We're learning from it and adapting it for Agor's multi-agent, spatial canvas paradigm.
...
**This is what we think AI agents should look like.**
...
_If you're running agor-openclaw or building your own agent loops in Agor, we'd love to hear about it. Open a discussion or drop us a note—your feedback shapes where this goes next._
