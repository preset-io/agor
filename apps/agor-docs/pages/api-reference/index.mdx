...
The response is the patched `Task` row with `status: 'running'`. The endpoint only accepts tasks in `created` or `queued` state on an `idle` session — busy sessions should be prompted via `POST /sessions/:id/prompt` instead, which creates and queues the task atomically.
