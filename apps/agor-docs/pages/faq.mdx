...
Agor uses **git worktrees** under the hood (same concept as `git worktree` command), but manages them for you automatically. When you create a branch in Agor, it:
...
**Agor manages the git worktrees for you** - no need to run `git worktree add` manually. Just create a branch in the UI or CLI, and Agor handles the git operations.
...
- **Branch**: `{{ worktree.name }}`, `{{ worktree.issue_url }}`, `{{ worktree.pull_request_url }}`
