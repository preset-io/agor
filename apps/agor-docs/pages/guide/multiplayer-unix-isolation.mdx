...
### 4b. (Optional) Enable the Web Terminal

The browser-based terminal (xterm.js modal) is disabled by default for every
role, including admins. To open it to `member`-role users and above, set:
...
- Terminal buttons are hidden in the UI when this flag is off.
...
>   `agor.db`, and the JWT secret. The daemon prints a startup warning when
...
>
> Enable `allow_web_terminal` only in `insulated` or `strict` modes for any
> deployment where you do not trust all members with daemon-level access.
