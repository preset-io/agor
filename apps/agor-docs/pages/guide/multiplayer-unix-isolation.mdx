...
- Worktree-level permissions still apply — opening a terminal tab against a
  worktree requires at least `session` permission on that worktree (when
  `worktree_rbac` is on).
...
- [Worktrees](/guide/worktrees) and [Sessions & Trees](/guide/sessions) — Core primitives
