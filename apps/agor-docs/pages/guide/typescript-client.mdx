...
`client.tasks.run()` calls `POST /tasks/:id/run` — the explicit "fire this task now" trigger. It's intended for `created`/`queued` tasks on idle sessions; busy sessions should be prompted via `client.sessions.prompt()` (which queues atomically).
