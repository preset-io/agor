---
title: One-Time Launch Authentication
description: Configure a generic external launch provider to authenticate users with short-lived one-time launch codes.
---

# One-Time Launch Authentication
...
This keeps long-lived runtime bearer tokens and external provider tokens out of browser URLs while still letting the runtime UI open directly into an authenticated session.
...
4. The exchange endpoint consumes the one-time code and returns a signed assertion for the authenticated subject.
...
Configure the launch provider under `external_launch` in `~/.agor/config.yaml`:
...
- `instance_id` or `runtime_instance_id`: must match the configured `instance_id`.

### Optional assertion claims

- `email`, `name`, `avatar` or `picture`.
- `role`: `viewer` or `member` by default; `admin` and `superadmin` are accepted only when `allow_admin_roles` is explicitly enabled, and `superadmin` is still capped unless runtime superadmin support is enabled.
- `provider`: stable provider label used in local identity mapping.
- `jti` or `nonce`: accepted for audit and correlation. One-time replay prevention remains the exchange endpoint's responsibility.

## Local user mapping

External launch users are mapped by stable external identity, not by email alone. The identity key is:

```text
(provider, issuer, subject)
```

If the same user returns with the same identity key, Agor reuses the existing local user. If a different external identity presents the same email address, Agor creates a separate local user instead of silently merging accounts.
...
- JWKS assertions must include a `kid` that matches a signing key.
- Map roles conservatively. By default, launch assertions cannot grant roles above `member`.
