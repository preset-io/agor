...
5. The daemon verifies issuer, audience, expiration, instance ID when present, and subject; then it maps or creates a local user by `(provider, issuer, subject)`.
...
external_launch:
  ...
  # Production: verify assertions with a JWKS endpoint or PEM public key.
...
- `role`: `viewer`, `member`, `admin`, or `superadmin` (superadmin is capped unless explicitly enabled)
- `provider`: stable provider label used in local identity mapping
- `instance_id` or `runtime_instance_id`: must match configured `instance_id` when present
...
- Assertions should be audience-bound to the runtime and expire quickly.
