...
While a normal worktree is _where_ code gets written, an Assistant is a _who_ — a long-lived entity with memory, identity, skills, and the ability to orchestrate work across your entire workspace. Assistants maintain file-based memory across sessions, run on heartbeat schedules, and coordinate multi-agent workflows through Agor's MCP tools.

Inspired by [OpenClaw](https://openclaw.ai/)'s agent loop architecture, Agor Assistants adapt the core patterns — markdown-based memory, identity files, scheduled loops — for team-based developer workflows with full visibility on spatial boards.

**Learn more:** See the full [Assistants guide](/guide/assistants) for setup, key concepts (BOOTSTRAP.md, BOARD.md, HEARTBEAT.md, Skills), and real-world examples.
