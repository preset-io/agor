...
Raspberry Pis, low-tier Codespaces, etc.).

The shipped scripts already set `NODE_OPTIONS=--max-old-space-size=4096` for
`@agor/core`'s `build`/`dev`, plus `packages/agor-live/build.sh`,
`docker/docker-entrypoint.sh`, and the dev Codespace entrypoint. If you build
from a custom path (e.g. running `tsup` directly, or wrapping the build in
your own script) and hit the OOM, set it yourself:
