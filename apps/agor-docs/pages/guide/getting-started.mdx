...
Agor stores that Codex login per user and reuses it for Codex sessions when no `OPENAI_API_KEY` overrides it.
...
This lets you mix approaches — use `claude setup-token` with your Max Pro plan, connect Codex through ChatGPT login, while teammates use per-user API keys.
