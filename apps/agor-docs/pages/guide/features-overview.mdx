...
description: A map of what Agor does — worktrees, sessions, boards, assistants, MCP server, rich chat UX, multiplayer, environments, scheduler, cards, artifacts, message gateway.
---
...
<img
  ...
  alt="Worktree card showing a parent coordinator session that spawned eight specialized security-review children running in parallel"
...
**Cron-style triggers for templated prompts.** Run a prompt every hour, every Monday, on a custom cadence. Targets a specific worktree or session. Powers [assistant](/guide/assistants) heartbeats, daily standups, scheduled audits, periodic backlog grooming.
