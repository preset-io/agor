...
description: Compare AI coding agent capabilities in agor. Feature matrix for Claude Code, Codex, Gemini, and GitHub Copilot integrations including streaming, tool approval, and SDK differences.
---
...
| Feature                     | Claude Code         | Codex                     | Gemini              | Copilot                   | Notes                                                                                   |
| --------------------------- | ------------------- | ------------------------- | ------------------- | ------------------------- | --------------------------------------------------------------------------------------- |
| **Streaming responses**     | ✅ Text + tools     | ⚠️ Tools only             | ✅ Text + tools     | ✅ Text + tools           | Claude, Gemini & Copilot stream text token-by-token; Codex streams tool events only     |
| **Stop mid-execution**      | ✅ Yes              | ⚠️ Limited                | ⚠️ Limited          | ⚠️ Limited                | Claude has `interrupt()`, others may lose partial work                                  |
| **Session import/export**   | ✅ Yes              | ❌ No                     | ❌ No               | ❌ No                     | Only Claude Code stores sessions as JSONL transcripts                                   |
...
| **MCP integration**         | ✅ Native           | ✅ Native                  | ✅ Native           | ✅ Native                 | All agents support MCP with Agor's built-in self-access server                          |
| **Permission requests**     | ✅ Rich             | ⚠️ Basic                  | ⚠️ Basic            | ✅ Callback-based         | Claude has granular tool permissions; Copilot uses `onPermissionRequest` callback        |
| **Project instructions**    | ✅ CLAUDE.md        | ✅ AGENTS.md               | ⚠️ Manual           | ⚠️ Manual                 | Claude auto-loads CLAUDE.md, Codex auto-loads AGENTS.md; Gemini/Copilot need injection  |
| **Tool execution**          | ✅ Rich widgets     | ⚠️ Basic                  | ⚠️ Basic            | ✅ Rich events            | Claude and Copilot expose detailed tool metadata for visualization                      |
| **Session continuity**      | ✅ SDK-managed      | ✅ History-based          | ✅ History-based    | ✅ SDK-managed            | Claude and Copilot use SDK session IDs; others replay message history                   |
| **Token usage tracking**    | ✅ Full             | ✅ Full                   | ❌ No               | ⚠️ Via events             | Claude & Codex surface SDK usage metadata; Copilot emits usage via turn events          |
| **Context window tracking** | 🟡 Estimated        | 🟡 Estimated              | ❌ No               | 🟡 Estimated              | Agor derives cumulative conversation usage from SDK metrics                             |
...
**Copilot:** ✅ **Callback-based permission system**
...
- Returns: `'approved'`, `'denied-interactively-by-user'`, or `'denied-by-rules'`
- In Agor's headless executor, all modes auto-approve (no interactive UI during execution)

**Agor Implementation:** Permission modals adapt based on available SDK metadata. Claude shows richest UX. Copilot's typed permission kinds enable future granular control.
...
- **Copilot permission UX** - Leverage typed permission kinds for granular approval UI
