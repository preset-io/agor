...
|---|---|---|---|
| `{{worktree.unique_id}}` | Branch | `3` | Auto-incrementing integer. Use for deterministic port offsets. |
| `{{worktree.name}}` | Branch | `feat-new-filter` | Slugified branch name. Good for `docker compose -p`. |
| `{{worktree.path}}` | Branch | `/home/you/agor/feat-new-filter` | Absolute path on disk. |
| `{{worktree.gid}}` | Branch | `1042` | Unix GID of the branch's isolation group. |
...
| `{{custom.<key>}}` | Branch custom context | `{{custom.compose_profile}}` | Anything you store in `worktree.custom_context`. |
...
`.agor.yml` lives at the **branch root** (`$WORKTREE_PATH/.agor.yml`) and is the shared, committed description of a repo's environment variants. It's a regular repo file: you edit it on a branch, open a PR, and it rolls out when merged.
...
| Action | Effect |
|---|---|
| **Import `.agor.yml`** | Parses `$WORKTREE_PATH/.agor.yml` and **replaces** `environment.variants` + `environment.default` in the DB. `template_overrides` and branch snapshots are untouched. |
...
4. Branch identity (`worktree.*`, `repo.*`).
