---
title: Boards & Zones
description: Boards are 2D spatial canvases for organizing worktrees. Zones are spatial regions that trigger templated prompts when worktrees are dropped into them.
---
...
Instead of linear lists or kanban columns, every [worktree](/guide/worktrees) lives at an `(x, y)` coordinate on a board. Sessions appear as a genealogy tree on the worktree card. Multiple users see the same canvas with live cursors.

![Agor board with custom-styled gradient zones organizing worktrees into a pipeline (MAIN SESH, Agor Coding Tasks, Create a pull request, Codex review, Human review). Each worktree card shows its session tree, env tabs, and PR badge.](/screenshots/board-hero.png)

_An Agor board with custom-styled zones forming a pipeline. Each worktree card shows its session tree, env tabs, and PR badge — and the floating PiP preview shows a session live-streaming as it works._
...
Drag a worktree into "Code Review" → auto-prompts the agent for review.
...
<img
  ...
  alt="Zone Trigger modal for a 'Codex review' zone — toggles between 'Create a new session' and 'Reuse a session', picks an action (Prompt / Fork / Spawn), and shows the editable templated prompt with mustache placeholders like {{ worktree.name }} and {{ worktree.pull_request_url }}"
...
Worktrees aren't the only thing that can live on a board. **[Cards](/guide/cards)** are generic workflow entities — support tickets, sales leads, content pieces — that an [assistant](/guide/assistants) creates and manages via MCP. Same spatial canvas, same zones, same multiplayer presence — just not git-bound.

---

## Related

- [Worktrees](/guide/worktrees) — The primary primitive that lives on boards
- [Sessions & Trees](/guide/sessions) — The genealogy rendered on each worktree card
- [Multiplayer & Social](/guide/multiplayer-social) — Cursors, facepile, comments
- [Cards](/guide/cards) — Non-worktree entities on boards
- [Assistants](/guide/assistants) — Agents that orchestrate worktrees and zones
