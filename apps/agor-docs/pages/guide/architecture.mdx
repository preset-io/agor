...
const { data: worktrees } = await agor.service('worktrees').find({
...
- **Branded ID types** — `WorktreeId`, `SessionId`, `BoardId` aren't interchangeable; the compiler catches mix-ups.
...
| Tool | Description | Example |
|------|-------------|---------|
| `agor_worktrees_get` | Get branch details | `{ worktreeId: '...' }` |
| `agor_worktrees_list` | List all branches | `{ limit: 10 }` |
| `agor_worktrees_create` | Create a branch with optional issue/PR links | `{ repoId: '019a...', worktreeName: 'feat-x', issueUrl: 'https://...' }` |
| `agor_worktrees_update` | Update branch metadata (issue/PR URLs, notes) | `{ worktreeId: '019a...', pullRequestUrl: 'https://...', notes: 'Ready for review' }` |
...
- `agor_worktrees_clone` - Clone an existing branch configuration (branch, metadata) for rapid duplication
