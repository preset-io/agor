...
Agor's worktree model is `git worktree add` off a single base clone at `~/.agor/repos/<owner>/<repo>/`. The base repo's `.git/config` is the canonical config for _all_ of its worktrees — `worktree.<name>.git` is a thin metadata directory under `.git/worktrees/<name>/`, but `remote.*`, `branch.*`, `http.*` all resolve through the shared base.
...
const targets = await glob(
  [`${homedir()}/.agor/repos/**/.git/config`, `${homedir()}/.agor/worktrees/**/.git/config`],
