...
- Interactive permission decisions still lack durable read-after-gap/replay. HA admits only modes with no Agor permission waiter. OpenCode remains task-blocked even in bypass mode because `question`/`task` effects still enter its local permission manager.
...
| Noninteractive task execution                    | Enabled per tool/mode gate: Claude bypass, Copilot bypass/allow-all, Gemini yolo, Codex `never`, current Cursor. OpenCode/interactive paths reject 503.                                                                                                                                           |
| Permission decisions                             | Unsupported: local waiter has no durable decision replay.                                                                                                                                                                                                                                         |
...
| Stateful MCP and process-affine auth/setup flows | Stateful MCP, MCP/GitHub/OpenCode setup, and Codex device polling remain blocked. Codex auth-file check/import/logout is admitted only with a consistent declared executor user home.                                                                                                             |
| Web terminal                                     | Startup blocked; no PTY bytes in Redis.                                                                                                                                                                                                                                                           |
| Managed environments                             | Local commands are blocked in favor of webhook-only execution. The pre-existing unfenced environment HealthMonitor is not constructed in HA; capability status reports `environmentHealthMonitor: false`.                                                                                         |
...
- four authenticated stateless MCP initialize requests through nginx alternated between both daemon upstreams, returned 200, and issued no `Mcp-Session-Id`; stateful MCP GET failed closed with the constrained-profile 503 response on both replicas;
...
- Durable permission decision read-after-gap/replay, then broader permission modes and OpenCode Task support.
- Stateful MCP, MCP OAuth, GitHub install state, Codex/OpenCode auth ownership, terminal, and Artifact runtime state externalization.
