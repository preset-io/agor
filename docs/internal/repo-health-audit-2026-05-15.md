...
Bundled into ~8 ready-to-cut PRs:

| # | Title | Touches | Source dimension |
|---|---|---|---|
...
| QW-4 | **Drop unused npm deps** — `@webscopeio/react-textarea-autocomplete` (+ its `.d.ts`) in `apps/agor-ui`; `@iarna/toml` in `packages/{core,executor}`; verify `@theguild/remark-mermaid` + `katex` in `apps/agor-docs`. | various package.jsons | Dead code |
...
| M-10 | **Audit `packages/client` published surface** | depcheck flags ALL `@feathersjs/*` + `socket.io-client` as unused — but this *is* the public client package. Either deps are vestigial or the package isn't actually using them at runtime. Users will hit whatever is wrong. | Dead code |
