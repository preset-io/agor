...
Agor now prevents the daemon environment from becoming the templated launcher's environment. After the launcher crosses into an external substrate, that substrate owns the executor's initial process environment and containment. Executor commands overlay the authenticated payload but can inherit substrate-provided runtime variables. Certification must prove that workload identity is non-exportable or otherwise safe for child processes; Agor cannot sanitize an opaque external launcher without an explicit descriptor contract.
...
- Templated/delegated launcher operators who implicitly depended on daemon environment credentials must move launcher authentication to substrate workload identity or an explicit out-of-band launcher mechanism before rollout. The authenticated executor payload remains on stdin.
