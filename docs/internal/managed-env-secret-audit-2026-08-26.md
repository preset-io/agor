...
- templated launcher shell: curated minimal host runtime allowlist plus the reserved operator-controlled `AGOR_CLOUD_*` launcher namespace; payload remains JSON stdin and never receives those ambient launcher credentials;
...
| Executor heartbeat callback                 | daemon operator hook                                            |                                         no |                                                no |                                                 no |                                no | command receives bounded heartbeat JSON on stdin under a secret-free environment                                                                    |
...
**Fix:** minimal ambient allowlists at `spawn-executor`, Codex auth dispatch, the heartbeat callback, and `@agor/git`; JSON stdin retained. The trusted templated launcher has one explicit operator-controlled exception for its reserved `AGOR_CLOUD_*` runtime-service namespace. The exception does not enter session env or the executor payload. Proxy URLs, provider tokens, Git identity, XDG paths, and account startup context are absent unless supplied through the authenticated user's explicit mapping. The daemon-resolved Git safety policy is the sole explicit session-process overlay.
...
Agor now prevents the daemon environment from becoming the templated launcher's environment, except for the documented operator-controlled `AGOR_CLOUD_*` launcher namespace. Those credentials terminate at the trusted launcher and are not included in executor payload/session env. After the launcher crosses into an external substrate, that substrate owns the executor's initial process environment and containment. Executor commands overlay ordinary authenticated payload variables but preserve substrate-owned `HOME`, `PATH`, identity, `LD_*`, and `DYLD_*` values. Certification must prove that workload identity and pod-owned credentials are non-exportable or otherwise safe for child processes; Agor cannot sanitize an opaque external launcher without an explicit descriptor contract.
...
- reserved `AGOR_CLOUD_*` delivery to the trusted templated launcher without launcher-credential payload/session-env disclosure;
...
- Templated/delegated launcher operators must use substrate workload identity, an explicit out-of-band launcher mechanism, or the reserved operator-controlled `AGOR_CLOUD_*` launcher contract documented above. Only that prefix is forwarded from the daemon ambient environment, and only to the trusted launcher; the authenticated executor payload remains on stdin without those ambient credentials. See [`preset-io/agor-cloud#198`](https://github.com/preset-io/agor-cloud/issues/198).
...
5. Fixed executors, credential helpers, operator callbacks, and Git children do not inherit the daemon's credential-bearing environment; trusted templated launcher shells receive only the documented `AGOR_CLOUD_*` operator exception in addition to the minimal runtime allowlist.
...
11. Decrypted task env exists only in the daemon launch scope, authenticated executor payload/stdin, and intended child process; it is not cached or written to a task/session row. Ambient launcher credentials exist only in the daemon and trusted launcher environments, never in that task env or payload.
