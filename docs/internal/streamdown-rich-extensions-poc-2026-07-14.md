...
- JSON objects only; maximum UTF-8 source size 100,000 bytes, nesting depth 32, 10,000 visited values, 2,000 items per array, 16 total nested composed views, and 2,000 px numeric width/height.
...
- At most four Vega fences can be activated in one Markdown document; a document above the limit fails closed to ordinary code. Completed charts load only within 400 px of the viewport.
...
| -------------------------------------------------------- | -----------: | -----------: | -----------------------------: |
| Callouts + Streamdown parity, Vega renderer removed      | 17,652,891 B |  4,000,387 B |                              — |
| Final demo-only POC with Vega registration and hardening | 17,744,326 B |  4,032,935 B | +91,435 B raw / +32,548 B gzip |
...
| Lazy chunk                              |           Raw |          Gzip |
| --------------------------------------- | ------------: | ------------: |
| `VegaLiteRenderer`                      |       7,093 B |       2,987 B |
| `vega` runtime                          |     653,271 B |     222,792 B |
| **Lazy total on first completed chart** | **660,364 B** | **225,779 B** |
...
- `pnpm --filter agor-ui test -- src/components/MarkdownRenderer/MarkdownRenderer.test.tsx src/components/MarkdownRenderer/VegaLiteRenderer.test.tsx src/components/MarkdownRenderer/vegaLiteSpec.test.ts src/components/MarkdownRenderer/vegaRendererLoader.test.ts`
  ...
  - covers default-off and aggregate fence gates, callouts, incomplete streaming, malformed/static-subset rejection, nested composition bounds, dark theme, both lazy/runtime timeouts, late finalization, control parity, accessibility, CSP mode, and blocked network loading
