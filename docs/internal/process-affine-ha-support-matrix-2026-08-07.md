...
| Claude subscription sign-in and logout    | PostgreSQL stores only state SHA-256 plus purpose-separated, tenant/user/attempt/generation-bound sealed PKCE and exact-route material. It owns DB-clock expiry, latest-wins generations, one atomic pending-to-exchanging claim, and bounded maintenance. Finalization, logout, replacement, and ordinary Claude method/API-key/token patches share the same tenant/user advisory lock and monotonic file tombstone. The authority-owning daemon performs and validates the exact-home write under cross-replica flock; authorization codes and tokens remain request-local. SQLite retains the process-local standalone flow.                                                                                                                                                                                                                                                                                           | **P+R** with a contained local writer                           | A peer can claim an unspent attempt after the issuer dies. Owner loss after the one-shot provider exchange or a possibly committed file/metadata write becomes ambiguous and is never replayed or path-deleted; a newer generation remains authoritative.                                                                                                         | claudeOAuth and claudeAuth are supported only when the capability proves sandbox, persistent-per-user, operator-verified cross-replica-flock, and the canonical exact-user home. Shared, overridden, and delegated homes remain gated. Never put the verifier, authorization code, tokens, or credential bytes in Redis.                                                                                                                                                                                                                                                                    |
