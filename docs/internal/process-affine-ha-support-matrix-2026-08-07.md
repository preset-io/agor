...
| Codex device auth                                | PostgreSQL stores a purpose-separated sealed device attempt bound to tenant, user, exact credential route, and generation. Database-time leases permit poll takeover, while one-shot exchange admission and the shared tenant/user mutation lock serialize completion, replacement, logout, and other credential choices. The authority-owning daemon writes the exact sandbox home through the cross-replica flock and monotonic file tombstone. SQLite retains the process-local standalone flow. Unlike Claude, the Codex provider runtime can still read and rewrite canonical `.codex/auth.json`; runtime-writer containment parity remains an explicit separate risk.                                                                                                                                                                                                                                               | **P+R** with fenced daemon writers; provider-runtime containment open | A peer can take over an expired poll lease before provider exchange. Loss after a possibly consumed exchange or file write becomes ambiguous and is not replayed automatically; a higher generation safely fences any stale daemon writer.                                                                                                                        | Existing support remains limited to a capability proving sandbox, persistent-per-user, and operator-verified cross-replica-flock on the canonical exact-user home. Shared, overridden, and delegated homes remain gated. This change does not claim to fix the provider-runtime writer or alter Codex behavior. Device codes, access tokens, refresh tokens, and credential bytes never go to Redis.                                                                                                                                                                                        |
| Claude subscription sign-in, refresh, and logout | PostgreSQL stores only state SHA-256 plus purpose-separated, tenant/user/attempt/generation-bound sealed PKCE and exact-route material. It owns DB-clock expiry, latest-wins generations, one atomic pending-to-exchanging claim, and bounded maintenance. Finalization, daemon-side refresh, logout, replacement, and ordinary Claude method/API-key/token patches share the same tenant/user advisory lock and monotonic file tombstone. Provider refresh I/O occurs outside locks; final source/route revalidation and generation byte-CAS yield to login/logout/route winners. The canonical refreshable file is masked from bubblewrap tasks, which receive only the short-lived access token through the sensitive executor channel. SQLite retains the process-local standalone attempt flow.                                                                                                                      | **P+R** with a contained local writer and runtime                     | A peer can claim an unspent attempt after the issuer dies or adopt a usable refresh winner. Owner loss after the one-shot provider exchange, rotating refresh, or possibly committed file/metadata write is never blindly replayed; a newer generation remains authoritative.                                                                                     | `claudeOAuth` and `claudeAuth` are true only when the capability proves the canonical exact-user per-user sandbox, concrete credential-file mask, and operator-verified cross-replica flock. Shared, unsafe/simple, overridden, and delegated homes remain gated. The default-off provider-policy flag independently gates OAuth endpoint/UI activation. Never put verifier, authorization code, refresh token, credential bytes, or runtime access tokens in Redis/logs.                                                                                                                   |
