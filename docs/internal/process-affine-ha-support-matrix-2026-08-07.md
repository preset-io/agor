...
| Codex device auth                                | PostgreSQL stores a purpose-separated sealed device attempt bound to tenant, user, exact credential route, and generation. Database-time leases permit poll takeover, while one-shot exchange admission and the shared tenant/user mutation lock serialize completion, replacement, logout, and other credential choices. The authority-owning daemon writes the exact sandbox home through the cross-replica flock and monotonic file tombstone. SQLite retains the process-local standalone flow.                                                                                                                                                                                                                                                                                                                                                                                                                       | **P+R** with a contained local writer                           | A peer can take over an expired poll lease before provider exchange. Loss after a possibly consumed exchange or file write becomes ambiguous and is not replayed automatically; a higher generation safely fences any stale writer.                                                                                                                               | Supported only when the capability proves sandbox, persistent-per-user, and operator-verified cross-replica-flock on the canonical exact-user home. Shared, overridden, and delegated homes remain gated. Device codes, access tokens, refresh tokens, and credential bytes never go to Redis.                                                                                                                                                                                                                                                                                              |
