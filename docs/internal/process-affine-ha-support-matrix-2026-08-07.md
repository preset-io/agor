...
| Claude subscription sign-in and logout    | PostgreSQL persists only state SHA-256 plus purpose-separated, tenant/user/attempt/generation-bound sealed PKCE material. It owns DB-clock expiry, latest-wins generations, and one atomic `pending -> exchanging` claim; every daemon runs bounded maintenance. The authorization code and tokens remain request-local. The credential write/logout still runs outside that authority and has no shared mutation lock or filesystem generation tombstone.                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Attempt: **P+R**; final mutation still **E+D**                  | A peer can claim an unspent attempt after the issuer dies. Loss after claim becomes `ambiguous` and is not replayed. However logout or a newer attempt can still race an already-running writer after its last DB check, and a delegated writer may outlive the daemon/DB claim owner.                                                                            | Keep `claudeOAuth` and `claudeAuth` gated in constrained HA. Reuse the generation-fenced cross-replica credential mutation coordinator and an exact-user containable route before activation; home durability/consistent bytes alone is insufficient. Never put verifier, code, tokens, or credential bytes in Redis.                                                                                                                                                                                                                                                                       |
