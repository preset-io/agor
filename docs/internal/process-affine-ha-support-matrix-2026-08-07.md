...
| ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MCP OAuth pending flow/callback           | Local Map contains state, PKCE verifier, client ID/secret, endpoints, tenant/user/server/mode, socket, and optional Promise resolvers (`apps/agor-daemon/src/register-services.ts:1215-1253,1399-1443`). Third-party callback is unauthenticated and locates state only in that Map (`register-services.ts:1493-1558`). TTL is 10 minutes; blocking callers time out after 5 (`register-services.ts:1255-1275,1415-1428`).                                                        | **P+D**; an exchange lease alone cannot close the provider-code crash gap | Callback to a non-owner or after owner death returns expired/not found. A blocked discover/test request dies. If death occurs after the provider consumes the authorization code but before PostgreSQL token persistence, safe automatic replay may be impossible. | Disable start/complete/callback-dependent OAuth flows in initial HA. Later persist an encrypted/sealed pending record with tenant/user/server binding, expiry, and atomic one-shot claim. Treat death during exchange as an explicit ambiguous terminal state unless provider idempotency exists. |
| OAuth completion events                   | Start can emit the authorization URL globally when no socket ID is available (`register-services.ts:1445-1451`). Completion uses unqualified user room for per-user mode and global emit for shared mode (`register-services.ts:1562-1593`).                                                                                                                                                                                                                                      | **S** only after security fix; otherwise **D**                            | UI can recover persisted token state by refetch, but in-flight waiter is local. Redis fanout would expose global auth URLs/state and shared completion metadata fleet-wide.                                                                                        | Never globally publish authorization URLs/state. Use tenant+user native rooms, clean room lifecycle, and let durable DB refetch be correctness.                                                                                                                                                   |
| Persisted MCP OAuth token + daemon cache  | Access/refresh/client credentials persist in PostgreSQL per server/user or shared row (`apps/agor-daemon/src/oauth-cache.ts:99-179`). The local cache holds a raw access token keyed only by MCP URL origin (`oauth-cache.ts:25-63`), not tenant/server/user.                                                                                                                                                                                                                     | PostgreSQL is **R**; cache is a security blocker                          | Owner death drops cache and peers can reload durable state. More seriously, a live multi-tenant daemon can return one tenant/user's cached token to another same-origin lookup.                                                                                    | Remove the cache or key it by trusted tenant + server + credential subject and prove negative isolation. Never place access/refresh/client secrets in Redis. Review encryption-at-rest separately.                                                                                                |
