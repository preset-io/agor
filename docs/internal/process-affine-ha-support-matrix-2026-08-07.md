...
| OAuth/GitHub third-party callback                | **No.** Redirects need not carry the originating affinity cookie.              | PostgreSQL covers GitHub install state; keep MCP OAuth disabled until its separate durable design lands.                 |
...
`0079`/`0082` history, no raw-state column, no implicit default-tenant RLS,
...
still-valid request target. Final post-rebase validation passes `pnpm check`,
103 focused core tests, 57 focused daemon tests, all 115 PostgreSQL tests across 21 files
