...
When a surface intentionally truncates user-supplied text, bound the available width and keep the
constraint intact through **every** shrinking flex or grid ancestor between that boundary and the
label. Flex items commonly need `min-width: 0`; grid tracks commonly need `minmax(0, 1fr)`. This
includes wrappers generated by component libraries: use the library's supported semantic
`styles` / `classNames` API rather than relying on an incidental DOM selector.

The label itself needs the single-line overflow rules and must be allowed to flex. Fixed neighbors
such as icons, badges, and counts should not shrink; apply that constraint to the neighbor's layout
root, not an inner indicator. AntD `Typography.Text` with
`ellipsis={{ tooltip: fullValue }}` supplies the label clamp and a hover tooltip only when the text
is clipped, but it does not constrain those ancestors for you.

Keep the full value in the control's accessible name. Do not add a nested tab stop merely to make
