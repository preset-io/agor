...
All IDs generated by Agor (Session, Task, Branch, Board, Repo, new User, etc.) are **UUIDv7** — RFC 9562, time-ordered. The first 48 bits encode the creation timestamp (ms precision).

Canonical identities already present in a database or supplied by an external
user authority remain stable. In particular, historical User rows may use a
lowercase full UUIDv4. Foreign keys to those rows must preserve that UUIDv4;
never translate an existing identity to a fabricated UUIDv7. Persistence
boundaries that accept an existing ID use `isCanonicalFullUuid`, while
`isValidUUID` remains the stricter UUIDv7 check for newly generated Agor IDs.
