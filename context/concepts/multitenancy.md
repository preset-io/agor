...
| Per-tenant browser link origin (`multi_tenancy.tenant_base_url_template`)                                                  | `resolveTenantBaseUrl` in `packages/core/src/config/multitenancy.ts`, applied by `getBaseUrl` in `config-manager.ts` from ambient tenant identity                     |
