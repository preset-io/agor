...
- `branch_permission_configs`, `branch_permission_entries`, and normalized session-sharing rows are authoritative when `branch_rbac` is enabled. A branch either inherits its board's entire template or uses one complete override. Historical owner/grant fields are inert compatibility shells.
...
- **Deleting a board** first materializes every inheriting branch as an override, including owner-authored session-sharing rules.
