...
| OC-27 | unit, cloud | Scratch volume filled during a turn (cloud) / checkpoint write fails (unit)                | Unit: turn reported `failed` ("checkpoint not durable"), nothing published. Cloud: the pod is `Evicted` by the kubelet size limit, the run fails, no new session pointer is recorded, and the next launch prunes the manifest-less attempt; the scratch root is the `AGOR_OPENCODE_SCRATCH_ROOT` mount, never `TMPDIR` |
| OC-33 | unit        | Stale Job (older launch pointer) reaches prune after a newer attempt was accepted          | The newer attempt survives; only entries older than the stale pointer are removed; the accepted pointer still resolves                                                                                                                                                                                                 |
