...
ALTER POLICY tenant_isolation_user_mcp_oauth_tokens ON user_mcp_oauth_tokens
 ...
 AND tenant_id=COALESCE(NULLIF(current_setting('agor.tenant_id',true),''),'default'));
--> statement-breakpoint
CREATE FUNCTION public.agor_mcp_managed_oauth_maintenance_tenants(after_tenant text DEFAULT NULL, page_size integer DEFAULT 100, requested_cell text DEFAULT NULL)
 ...
 BEGIN
  IF current_setting('agor.system_scope',true) IS DISTINCT FROM 'mcp_oauth_maintenance'
    OR page_size IS NULL OR page_size<1 OR page_size>100 OR length(after_tenant)>1024
    OR (requested_cell IS NOT NULL AND requested_cell !~ '^[A-Za-z0-9_-]{1,128}$') THEN
  ...
  RETURN QUERY SELECT candidates.tenant_id FROM (
   (SELECT DISTINCT p.tenant_id COLLATE "C" AS tenant_id FROM public.mcp_oauth_pending_flows p
    WHERE p.credential_origin='cloud_managed_v1' AND p.status IN ('pending','exchanging')
      AND (requested_cell IS NULL OR p.managed_metadata->'owner'->>'cell_id'=requested_cell)
   ...
   (SELECT DISTINCT t.tenant_id COLLATE "C" AS tenant_id FROM public.user_mcp_oauth_tokens t
    WHERE t.credential_origin='cloud_managed_v1'
      AND (requested_cell IS NULL OR t.managed_metadata->'owner'->>'cell_id'=requested_cell) AND (after_tenant IS NULL OR t.tenant_id COLLATE "C">after_tenant COLLATE "C") ORDER BY 1 LIMIT page_size)
   UNION
   (SELECT DISTINCT o.tenant_id COLLATE "C" AS tenant_id FROM public.mcp_managed_oauth_outbox o
    WHERE o.completed_at IS NULL
      AND (requested_cell IS NULL OR o.managed_metadata->'owner'->>'cell_id'=requested_cell) AND (after_tenant IS NULL OR o.tenant_id COLLATE "C">after_tenant COLLATE "C") ORDER BY 1 LIMIT page_size)
...
CREATE POLICY managed_oauth_owner_routing_select ON user_mcp_oauth_tokens FOR SELECT USING (
 ...
 AND pg_catalog.pg_has_role(current_user,(SELECT proowner FROM pg_catalog.pg_proc WHERE oid='public.agor_mcp_managed_oauth_maintenance_tenants(text,integer,text)'::regprocedure),'MEMBER'));
--> statement-breakpoint
...
CREATE POLICY managed_oauth_owner_routing_select ON mcp_managed_oauth_outbox FOR SELECT USING (
 ...
 AND pg_catalog.pg_has_role(current_user,(SELECT proowner FROM pg_catalog.pg_proc WHERE oid='public.agor_mcp_managed_oauth_maintenance_tenants(text,integer,text)'::regprocedure),'MEMBER'));
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.agor_mcp_managed_oauth_maintenance_tenants(text,integer,text) FROM PUBLIC;
--> statement-breakpoint
...
GRANT EXECUTE ON FUNCTION public.agor_mcp_managed_oauth_maintenance_tenants(text,integer,text) TO PUBLIC;
--> statement-breakpoint
