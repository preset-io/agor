...
CREATE TABLE `__new_gateway_provider_actions` (
  ...
  CONSTRAINT `gateway_provider_actions_kind_check` CHECK (`kind` IN ('deliver_message', 'discord_progress', 'discord_notice')),
  CONSTRAINT `gateway_provider_actions_shape_check` CHECK ((`kind` = 'deliver_message' AND `message_id` IS NOT NULL AND `drop_after` IS NULL) OR (`kind` = 'discord_progress' AND `message_id` IS NULL AND ((json_extract(`params`, '$.state') = 'done' AND `drop_after` IS NULL) OR (json_extract(`params`, '$.state') <> 'done' AND `drop_after` IS NOT NULL))) OR (`kind` = 'discord_notice' AND `message_id` IS NULL AND `thread_session_map_id` IS NULL AND `session_id` IS NULL AND `task_id` IS NULL AND (`gateway_inbound_event_id` IS NOT NULL OR `status` IN ('completed', 'dead_letter', 'canceled')) AND `drop_after` IS NOT NULL)),
...
SELECT
  ...
  `provider_installation_id`, `provider_config_generation`, `kind`, `idempotency_key`,
  `thread_session_map_id`, `session_id`, `task_id`, `message_id`, `gateway_inbound_event_id`,
  `params`, `status`, `attempts`, `not_before`,
  CASE WHEN `kind` = 'discord_notice' AND `drop_after` IS NULL THEN `created_at` + 120000 ELSE `drop_after` END,
  `claim_token`, `claim_generation`, `claim_expires_at`, `claim_listener_token`,
  `claim_listener_generation`, `claim_instance_id`, `claim_boot_id`,
  `last_error_code`, `execution_metadata`, `result_metadata`, `completed_at`,
  `dead_lettered_at`, `canceled_at`
