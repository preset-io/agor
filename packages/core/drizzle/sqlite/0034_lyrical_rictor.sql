...
CREATE TABLE `__new_worktrees` (
	...
	`others_can` text DEFAULT 'session',
	`unix_group` text,
	`others_fs_access` text DEFAULT 'read',
	`data` text NOT NULL,
...
INSERT INTO `__new_worktrees`("worktree_id", "repo_id", "created_at", "updated_at", "created_by", "name", "ref", "ref_type", "worktree_unique_id", "start_command", "stop_command", "nuke_command", "health_check_url", "app_url", "logs_command", "board_id", "schedule_enabled", "schedule_cron", "schedule_last_triggered_at", "schedule_next_run_at", "needs_attention", "archived", "archived_at", "archived_by", "filesystem_status", "others_can", "unix_group", "others_fs_access", "data") SELECT "worktree_id", "repo_id", "created_at", "updated_at", "created_by", "name", "ref", "ref_type", "worktree_unique_id", "start_command", "stop_command", "nuke_command", "health_check_url", "app_url", "logs_command", "board_id", "schedule_enabled", "schedule_cron", "schedule_last_triggered_at", "schedule_next_run_at", "needs_attention", "archived", "archived_at", "archived_by", "filesystem_status", "others_can", "unix_group", "others_fs_access", "data" FROM `worktrees`;--> statement-breakpoint
