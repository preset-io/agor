...
JOIN `mcp_servers` winner ON winner.`mcp_server_id` = (
  ...
  WHERE x.`source`='catalog' AND x.`owner_user_id`=loser.`owner_user_id` AND x.`catalog_entry_name`=loser.`catalog_entry_name`
...
DELETE FROM `session_mcp_servers` WHERE `mcp_server_id` IN (
 SELECT loser.`mcp_server_id` FROM `mcp_servers` loser WHERE loser.`source`='catalog' AND loser.`catalog_entry_name` IS NOT NULL AND loser.`mcp_server_id` <> (
  SELECT x.`mcp_server_id` FROM `mcp_servers` x WHERE x.`source`='catalog' AND x.`owner_user_id`=loser.`owner_user_id` AND x.`catalog_entry_name`=loser.`catalog_entry_name` ORDER BY COALESCE(x.`updated_at`,x.`created_at`) DESC,x.`mcp_server_id` DESC LIMIT 1));--> statement-breakpoint
DELETE FROM `mcp_servers` WHERE `source`='catalog' AND `catalog_entry_name` IS NOT NULL AND `mcp_server_id` <> (
 SELECT x.`mcp_server_id` FROM `mcp_servers` x WHERE x.`source`='catalog' AND x.`owner_user_id`=`mcp_servers`.`owner_user_id` AND x.`catalog_entry_name`=`mcp_servers`.`catalog_entry_name` ORDER BY COALESCE(x.`updated_at`,x.`created_at`) DESC,x.`mcp_server_id` DESC LIMIT 1);--> statement-breakpoint
CREATE UNIQUE INDEX `mcp_servers_catalog_owner_uq` ON `mcp_servers` (`owner_user_id`,`catalog_entry_name`) WHERE `source`='catalog' AND `catalog_entry_name` IS NOT NULL;
