...
CREATE TABLE `__new_gateway_provider_actions` (
  ...
  CONSTRAINT `gateway_provider_actions_shape_check` CHECK ((`kind` = 'deliver_message' AND `message_id` IS NOT NULL AND `drop_after` IS NULL) OR (`kind` = 'discord_progress' AND `message_id` IS NULL AND ((json_extract(`params`, '$.state') = 'done' AND `drop_after` IS NULL) OR (json_extract(`params`, '$.state') <> 'done' AND `drop_after` IS NOT NULL))) OR (`kind` = 'discord_notice' AND `message_id` IS NULL AND `thread_session_map_id` IS NULL AND `session_id` IS NULL AND `task_id` IS NULL AND (`gateway_inbound_event_id` IS NOT NULL OR `status` IN ('completed', 'dead_letter', 'canceled')) AND `drop_after` IS NULL)),
