...
const tenantTest = (title: string, work: (fixtures: { db: Database }) => Promise<void>) =>
  ownedDbTest(title, async ({ db }) => runWithTenantContext('default', () => work({ db })));

tenantTest(
  'drains a large single session in bounded transactions, preserves shared neighbors and keeps branch last',
  async ({ db }) => {
    ...
    expect(
      (await select(db).from(messages).where(eq(messages.message_id, foreignMessage)).one())
        ?.task_id
    ).toBeNull();
...
tenantTest(
  'reference reconciliation clears exact structured pointers but leaves arbitrary content unchanged',
  async ({ db }) => {
...
tenantTest(
  'completed reference scan fences late teammate dependencies and namespace ownership changes',
  async ({ db }) => {
    ...
    await expect(branches.update(neighbor.branch_id, { custom_context })).rejects.toThrow(
      'deletion'
    );
    await expect(
      branches.create({
