...
describe('MessagesRepository JSON and edge cases', () => {
  ...
  dbTest(
    'should truncate oversized tool_result payloads while preserving message structure',
    async ({ db }) => {
      const messages = new MessagesRepository(db);
      const sessionId = await createTestSession(db);
      const originalLimit = process.env.AGOR_MESSAGES_MAX_TOOL_RESULT_BYTES;
      process.env.AGOR_MESSAGES_MAX_TOOL_RESULT_BYTES = '256';
      ...
      try {
        ...
        const created = await messages.create(data);
        const blocks = created.content as Array<Record<string, unknown>>;
        const toolResult = blocks[0];
        const storedContent = toolResult?.content;

        expect(typeof storedContent).toBe('string');
        expect((storedContent as string).length).toBeLessThan(largeResult.length);
        expect(storedContent).toContain('[agor: tool payload truncated for storage]');
      } finally {
        if (originalLimit === undefined) {
          delete process.env.AGOR_MESSAGES_MAX_TOOL_RESULT_BYTES;
        } else {
          process.env.AGOR_MESSAGES_MAX_TOOL_RESULT_BYTES = originalLimit;
        }
      }
    }
  );
  ...
  dbTest(
    'should truncate oversized tool_use input payloads in both content and tool_uses',
    async ({ db }) => {
      const messages = new MessagesRepository(db);
      const sessionId = await createTestSession(db);
      const originalLimit = process.env.AGOR_MESSAGES_MAX_TOOL_INPUT_BYTES;
      process.env.AGOR_MESSAGES_MAX_TOOL_INPUT_BYTES = '256';

      try {
        const veryLargeInput = { query: 'y'.repeat(3000) };
        const data = createMessageData({
          session_id: sessionId,
          role: MessageRole.ASSISTANT,
          type: 'assistant',
        ...
        const created = await messages.create(data);
        const contentBlocks = created.content as Array<Record<string, unknown>>;
        const toolUseInput = contentBlocks[0]?.input as Record<string, unknown>;
        const summaryInput = created.tool_uses?.[0]?.input as Record<string, unknown>;

        expect(toolUseInput.__agor_truncated).toBe(true);
        expect(summaryInput.__agor_truncated).toBe(true);
        expect(toolUseInput.field).toBe('input');
        expect(summaryInput.field).toBe('input');
      } finally {
        if (originalLimit === undefined) {
          delete process.env.AGOR_MESSAGES_MAX_TOOL_INPUT_BYTES;
        } else {
          process.env.AGOR_MESSAGES_MAX_TOOL_INPUT_BYTES = originalLimit;
        }
      }
    }
  );
