...
describe.skipIf(!postgresUrl)('runtime recovery PostgreSQL RLS', () => {
  let db: Database;
  ...
  it('discovers active and incomplete work only, then preserves tenant isolation', async () => {
    ...
    await runWithTenantDatabaseScope(db, tenantB, async (scoped) => {
      ...
      expect(await new ThreadSessionMapRepository(scoped).findBySession(b.sessionId)).toMatchObject(
        {
          id: b.mappingId,
        }
      );
...
describe.skipIf(!postgresOwnerUrl)('terminal consequence PostgreSQL migration', () => {
  it('baselines legacy terminal Tasks in two tenants without weakening FORCE RLS', async () => {
    const client = postgres(postgresOwnerUrl!, { max: 1, prepare: false });
    const schema = `terminal_recovery_${generateId().replaceAll('-', '').slice(0, 20)}`;
    const migration = await readFile(
      new URL('../../../drizzle/postgres/0083_terminal_consequence_recovery.sql', import.meta.url),
      'utf8'
    );
