...
describe('divergent pre-main managed migration ledger', () => {
  it('refuses the applied feature timestamp collision without rewriting history or running SQL', async () => {
    const db = createDatabase({ dialect: 'sqlite', url: ':memory:' });
    try {
      await executeRaw(
        db,
      ...
      const source = await readFile(
        new URL('../../drizzle/sqlite/0112_mcp_managed_oauth_authority.sql', import.meta.url)
      );
      const hash = createHash('sha256').update(source).digest('hex');
      await executeRaw(
        db,
        sql`INSERT INTO __drizzle_migrations VALUES (1, ${hash}, 1789344000005)`
      );
      const before = rawRows(await executeRaw(db, sql`SELECT * FROM __drizzle_migrations`));
      await expect(checkMigrationStatus(db)).rejects.toThrow('explicit reviewed forward migration');
      await expect(runMigrations(db, { allowOfflineCutover: true })).rejects.toThrow(
        'Migration failed'
      );
      expect(rawRows(await executeRaw(db, sql`SELECT * FROM __drizzle_migrations`))).toEqual(
        before
      );
      expect(
        rawRows(await executeRaw(db, sql`SELECT name FROM sqlite_master WHERE type='table'`))
      ).toEqual([{ name: '__drizzle_migrations' }]);
    } finally {
      (db as unknown as { $client: ReturnType<typeof createClient> }).$client.close();
    }
  });
});
