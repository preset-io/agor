...
describe('Postgres migrations', () => {
  ...
  it('assigns GitHub install state unique post-HA migration watermarks', async () => {
    ...
    for (const [journal, expectedTag, expectedIndex] of [
      ...
      const entryIndex = journal.entries.findIndex((candidate) => candidate.tag === expectedTag);
      const entry = journal.entries[entryIndex];
      const predecessor = journal.entries[entryIndex - 1];
...
describe('terminal consequence recovery migration', () => {
  it('baselines only pre-contract terminal Tasks in SQLite', async () => {
    ...
    try {
      ...
      const migration = await readFile(
        new URL('../../drizzle/sqlite/0086_terminal_consequence_recovery.sql', import.meta.url),
        'utf8'
      );
