...
function migrationTenantTables(): string[] {
  ...
  const gatewayProviderActionsMigration = readRepoFile(
    'packages/core/drizzle/postgres/0089_gateway_provider_actions.sql'
  );
  ...
  return [
    ...new Set(
      [
        ...
        ...gatewayProviderActionsMigration.matchAll(
...
function rlsPolicyTables(): string[] {
  const migration = [
    ...
    readRepoFile('packages/core/drizzle/postgres/0089_gateway_provider_actions.sql'),
  ].join('\n');
  const retiredTables = retiredTenantTables();
  return [
    ...new Set(
