...
describe('PermissionService.waitForDecision', () => {
  ...
  it('should return decision when resolved', async () => {
    ...
    const waitPromise = service.waitForDecision(
      requestId,
      taskId,
  ...
  it('should handle multiple concurrent requests', async () => {
    ...
    const req1 = service.waitForDecision(
      'req-1',
      'task-1' as TaskID,
      'session-test' as SessionID,
      signal
    );
    const req2 = service.waitForDecision(
      'req-2',
      'task-2' as TaskID,
      'session-test' as SessionID,
      signal
    );
    const req3 = service.waitForDecision(
      'req-3',
      'task-3' as TaskID,
      'session-test' as SessionID,
      signal
  ...
  it('should preserve decision metadata', async () => {
    ...
    expect(result.scope).toBe(PermissionScope.PROJECT);
