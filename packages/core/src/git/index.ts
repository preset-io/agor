...
/**
 * Path to the agor-git wrapper script
 */
function getGitWrapperPath(): string {
  return join(tmpdir(), 'agor-git-wrapper.sh');
}

/**
 * Ensure the sudo git wrapper script exists
 *
 * Creates a wrapper script that runs git via `sudo su -` to get fresh group memberships.
 * This is necessary because Unix groups are cached at login time, so the daemon process
 * won't have access to newly-created groups without a fresh login shell.
 *
 * Uses the escapeShellArg utility from command-executor for safe argument handling.
 */
function ensureGitWrapper(targetUser: string): string {
  const wrapperPath = getGitWrapperPath();
  ...
  const wrapperScript = [
    ...
    '# Generated by @agor/core - do not edit manually',
    '#',
    '# This wrapper solves the Unix group caching problem:',
    "# When a user is added to a group, running processes don't pick up",
    '# the new membership until they get a fresh login shell.',
    '# Using sudo su - creates that fresh login shell.',
    '',
    ...
    'done',
    '',
    '# Run git with a fresh login shell to pick up current group memberships',
    '# -n prevents password prompts (requires passwordless sudo)',
    'exec sudo -n su - "$TARGET_USER" -c "$CMD"',
  ].join('\n');
  ...
  if (existsSync(wrapperPath)) {
    try {
      ...
      needsWrite = existing !== wrapperScript;
    } catch {
      needsWrite = true;
    }
  }
  ...
  if (needsWrite) {
    ...
    console.debug(`[Git] Created sudo wrapper at ${wrapperPath} for user ${targetUser}`);
  }
 ...
 *
 * When RBAC is enabled, automatically uses a sudo wrapper to run git as the daemon user
 * with fresh group memberships. This solves the Unix group caching issue where newly
 * created groups aren't available to running processes.
 *
...
function createGit(baseDir?: string, env?: Record<string, string>) {
  ...
  // Use sudo wrapper when RBAC is enabled and we have a daemon user
  const useSudoWrapper = rbacEnabled && daemonUser;

  let gitBinary: string;

  if (useSudoWrapper) {
    gitBinary = ensureGitWrapper(daemonUser);
    console.debug(`[Git] Using sudo wrapper as user: ${daemonUser}`);
  } else {
