...
try {
  ...
  const existingMemberClaudeAuth = await fetch(`${daemonA}/check-auth`, {
    method: 'POST',
    headers: {
      authorization: `Bearer ${memberAccessToken}`,
      'content-type': 'application/json',
  ...
  assert.equal(existingMemberClaudeAuth.status, 201);
  const existingMemberClaudeStatus = (await existingMemberClaudeAuth.json()).status;
  if (existingMemberClaudeStatus === 'authenticated') {
    console.log(
      "ok - preserved the harness member's pre-existing Claude credential instead of overwriting it"
    );
  } else {
    ...
    // the winning attempt, and advances the exact home's tombstone.
    const selectNative = await fetch(`${daemonB}/users/${memberUserId}`, {
      method: 'PATCH',
      headers: {
        authorization: `Bearer ${memberAccessToken}`,
        'content-type': 'application/json',
    ...
    assert.match(memberTenantId, /^[A-Za-z0-9_-]+$/);
    assert.match(memberUserId, /^[A-Za-z0-9_-]+$/);
    const claudeConfigDir = `/home/agor/.agor/tenants/${memberTenantId}/homes/${memberUserId}/.claude`;
    ...
    assert(
      !claudeAuthLogs.includes(dummyClaudeRefresh),
